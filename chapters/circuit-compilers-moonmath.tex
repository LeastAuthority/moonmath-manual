...
As we have seen in chapter \ref{sec:statements} and in \ref{R1CS},\sme{check references}, both arithmetic circuits and rank-1 constraint systems have a modularity property by which it is possible to synthesize complex circuits from simple ones. A basic approach taken by many circuit/R1CS compilers is therefore to provide a library of atomic and simple circuits and then define a way to combine those basic building blocks into arbitrary complex systems.
...
During that process, edge labels are drawn according to the rules from \ref{def:algebraic-circuit}\sme{check reference}. If the associated variable represents a private value, we use the $W$ label to indicate a witness, and if it represents a public value, we use the $I$ label to indicate an instance.
...
\begin{example}[A trivial Circuit]\label{ex:trivial-circuit} To give an intuition of how to write and compile circuits in the \texttt{PAPER} language, consider the following statement description:
\begin{lstlisting}
...
\end{equation}
Circuit compilers have to compile these functions into algebraic gates, as explained in \ref{algebraic-gates}\sme{check reference}. Every other function has to be expressed in terms of them and proper wiring.
...
\end{center}
\end{example} 
\begin{example}[$3$-factorization] Consider our $3$-factorization problem from example \ref{ex:3-factorization}\sme{check reference} and the associated circuit $C_{3.fac\_zk}(\F_{13})$ we provided in equation \ref{C3fac}\sme{check reference}. To understand the process of replacing high-level functions by their associated circuits inductively, we want define a \texttt{PAPER} statement that we brain-compile into an algebraic circuit equivalent to $C_{3.fac\_zk}(\F_{13})$:
\begin{lstlisting}
...
\end{center}
Using the general method from \ref{R1CS}\sme{check reference}, the circuits associated rank-1 constraint system is given by:
\begin{equation}
...
\end{lstlisting}
In the setup phase of a statement, we compile every occurrence of the $\mathtt{SUB}$ function into an instance of its associated subtraction circuit, and edge labels are generated according to the rules from \ref{def:algebraic-circuit}\sme{check reference}.
...
If the underlying field is a prime field, one approach would be to use Fermat's little theorem \ref{fermats-little-theorem}\sme{check reference} to compute the multiplicative inverse inside the circuit. To see how this works, let $\F_p$ be the prime field. The multiplicative inverse $x^{-1}$ of a field element $x\in\F$ with $x\neq 0$ is then given by $x^{-1}= x^{p-2}$, and computing $x^{p-2}$ in the circuit therefore computes the multiplicative inverse. 
...
\end{center}
Using the general method from \ref{R1CS}\sme{check reference}, the circuit is transformed into the following rank-1 constraint system:
\begin{equation}
S_1 \cdot S_2 = 1
\end{equation}
...
\end{center} 
Using the method from \ref{R1CS}\sme{check reference}, we transform this circuit into the following rank-1 constraint system:
\begin{align*}
S_2 \cdot S_3 &= 1\\
S_1 \cdot S_3 &= S_4
\end{align*}
...
\begin{exercise} Let $\mathtt{F}$ be the field $\F_5$ of modular $5$ arithmetics from example \ref{primfield_z_5}\sme{check reference}. Brain-compile the following \texttt{PAPER} statement into an algebraic circuit:
\begin{lstlisting}
...
\end{lstlisting}
\end{exercise}
\begin{exercise} Consider the tiny-jubjub curve from example \ref{TJJ13}\sme{check reference} and its associated circuit \ref{ex:trivial-circuit}\sme{check reference}. Write a statement in \texttt{PAPER} that brain-compiles the statement into a circuit equivalent to the one derived in XXX\sme{add reference}, assuming that curve points are instances and every other assignment is a witness. 
\end{exercise}
...
\end{center}
The associated rank-1 constraint system can be deduced from the general process \ref{R1CS}\sme{check reference} and consists of the following constraint:
\begin{equation}
 S_1 \cdot S_2 = S_3
\end{equation}
...
\end{center}
The associated rank-1 constraint system can be deduced from the general process \ref{R1CS}\sme{check reference} and consists of the following constraints:
\begin{align*}
S_1 \cdot S_2 & = S_3\\
...
\end{exercise}
\paragraph{Modularity} As we have seen in chapter \ref{sec:statements},\sme{check references}, both algebraic circuits and R1CS have a modularity property, and as we have seen in this section, all basic boolean functions are expressible in circuits. Combining those two properties, show that it is possible to express arbitrary boolean functions as algebraic circuits.

This shows that the expressiveness of algebraic circuits and therefore rank-1 constraint systems is as general as the expressiveness of boolean circuits. An important implication is that the languages $L_{R1CS-SAT}$ and $L_{Circuit-SAT}$ as defined in \ref{r1cs-satisfiability}\sme{check reference}, are as general as the famous language $L_{3-SAT}$, which is known to be $\mathcal{NP}$-complete. 
...
\end{center}\sme{can we rotate this by $90^{\circ}$?}
...
To compute the associated R1CS, we can use the general method from \ref{R1CS}\sme{check reference} and look at every labeled outgoing edge not coming from a source node. We declare the edges coming from input nodes as well as the edge going to the single output node as public, and every other edge as private input. In this case we get:
\begin{align*}
...
\end{lstlisting}
During the setup-phase, a circuit compiler might then replace any occurrence of the \texttt{uN} type by \texttt{N} variables of \texttt{boolean} type. Using the definition of booleans, each of these variables is then transformed into the \texttt{field} type and a boolean constraint system. To see how this can be achieved, we use \texttt{PAPER} as an example and get the following circuit:
\begin{center}
...
We can use the general technique from \ref{R1CS}\sme{check reference} to derive the associated rank-1 constraint system of the conditional assignment operator. We get the following:
\begin{align*}
S_1 \cdot S_2 & = S_3 \\
(1 - S_1) \cdot S_4 & = S_5 \\
(S_3 + S_5)\cdot 1 &= S_6
...
\subsubsection{Loops} In many programming languages, various loop control structures are defined that allow developers to execute expressions with a specified number of repetitions or arguments. In particular, it is often possible to implement unbounded loops like the loop structure give below, where the number of executions depends on execution inputs and is therefore unknown at compile time:
\begin{lstlisting}
  while true do { }
\end{lstlisting}

\sme{Add example}\smelong{M: Add another example where the loop actually depends on the input.}
...
\paragraph{Twisted Edwards curve constraints} As we have seen in section \ref{sec:edwards}\sme{check reference}, a twisted Edwards curve over a finite field $F$ is defined as the set of all pairs of points $(x,y)\in \F\times \F$ such that $x$ and $y$ satisfy the equation $a\cdot x^2+y^2= 1+d\cdot x^2y^2$. As we have seen in example XXX\sme{add reference}, we can transform this equation into the following circuit:
\begin{center}
