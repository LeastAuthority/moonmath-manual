...
\end{lstlisting}
\end{exercise}
\begin{exercise} Consider the tiny-jubjub curve from example \ref{PJJ13}\sme{check reference} and its associated circuit \ref{ex:trivial-circuit}\sme{check reference}. Write a statement in \texttt{PAPER} that brain-compiles the statement into a circuit equivalent to the one derived in XXX\sme{add reference}, assuming that curve points are instances and every other assignment is a witness. 
\end{exercise}
