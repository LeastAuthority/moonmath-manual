...
sage: F5(-16)*(F5(4)*a^3+F5(27)*b^2) != F5(0)
...
\end{equation}
This map is a 1:1 correspondence, and its inverse map is given by mapping the point at infinity onto the point at infinity, and mapping each curve point $(x,y)$ onto the curve point $(c^{-2}x,c^{-3}y)$.
\begin{example}
...
\\
\\
y'  & = \left(\frac{3x^2+a}{2y}\right)\left(x-x'\right) - y  & \text{\# insert points} \\
    & = \left(\frac{3\cdot 4^2+1}{2\cdot 2}\right)\left(4-3\right) - 2 & \text{\# simplify in } \F_5\\
    & = 1\cdot 1 + 3
      = 4
\end{align*}
