...
Consider the line which is tangential to the elliptic curve at $P$, in the sense that it ``just touches'' the curve at that point. If this line intersects the elliptic curve at a second point $R'$, the sum $P\oplus P$ is the reflection of $R'$ at the $x$-axis. If it does not intersect the curve at a second point, define the sum to be the point at infinity $\Oinf$. Calling such a line a \term{tangent}, it can be shown that no such tangent will intersect the curve in more than two points. This implies that doubling is not ambiguous.
\end{itemize}
\end{definition}
...
As we have seen in the previous section, elliptic curves $E(\F)$ have the structure of a commutative group associated to them. It can be shown that this group is finite whenever the underlying field $\F$ is finite, but it is not necessarily cyclic. As we know from \eqref{scalarmultiplication}, this implies that there is a notion of scalar multiplication associated to any cyclic subgroup of an elliptic curve over finite fields.

To understand this scalar multiplication, recall from \defname{} \ref{cyclic-groups} that every finite cyclic group of order $n$ has a generator $g$ and an associated exponential map $g^{(\cdot)}: \Z_n \to \G$, where $g^x$ is the $x$-fold product of $g$ with itself. For any point $P$ of finite order $n$ on an elliptic curve, the subgroup $\langle P\rangle$ is cyclic and the map $[\,\cdot\,]P:\Z_n\to \langle P\rangle$ is defined analogously.

Elliptic curve scalar multiplication is the exponential map written in additive notation. To be more precise, let $\F$ be a finite field, $E(\F)$ an elliptic curve, and $P\in E(\F)$ a point of order $n$. Then the \term{elliptic curve scalar multiplication} with base $P$ is defined as follows (where $[0]P = \Oinf$ and $[m]P = P+P+\ldots + P$ is the $m$-fold sum of $P$ with itself ):
$$
...
In the following example, we look at some implications of the fact that this particular elliptic curve group is finite and cyclic and apply the logarithmic order.
...
To compute the embedding degrees $k$, we can use Sage and loop through small embedding degrees until we find a match. We compute as follows:
\begin{sagecommandline}
...
As we have seen in section \ref{sec:hashing-to-groups}, some general methods are known for hashing into finite cyclic groups. Since elliptic curves over finite fields are finite commutative groups, those methods can be utilized by hashing into a chosen cyclic subgroup (often followed by cofactor clearing). However, in what follows we want to describe some methods specific to elliptic curves that are frequently used in real-world applications. 
...
We know that elliptic curve groups over finite fields are finite commutative groups. Therefore, an interesting question is whether it is possible to estimate the number of elements that this curve contains. Since an affine \concept{short Weierstrass} curve consists of pairs $(x,y)$ of elements from a finite field $\F_q$ plus the point at infinity, and the field $\F_q$ contains $q$ elements, the number of curve points cannot be arbitrarily large, since it can contain at most $q^2+1$ many elements. 
