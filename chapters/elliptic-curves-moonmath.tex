...
\end{center}
As we will see in what follows, this curve is rather special, as it is possible to represent it in two alternative forms, called the \term{Montgomery} and the \term{twisted Edwards form} (See XXX\sme{add reference} and XXX\sme{add reference}).
\end{example}
...
To see if $4$ is a quadratic residue, we can use Euler's criterion (\ref{eq: Euler_criterium})\sme{check reference} to compute the Legendre symbol of $4$. We get the following:
...
\paragraph{The $BLS6\_6$ pen-and-paper curve}
