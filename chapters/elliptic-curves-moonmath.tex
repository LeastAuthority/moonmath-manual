...
Note that in the literature, the set $E/\F_q$ that included $\mathcal{O}$ is often called the set of \textit{rational points} of the elliptic curve. However in what follows we will frequently identify an elliptic curve with its set of rational points.
...
\begin{example}Consider our prime field $F_5$ from (XXX). If we choose $a=1$ and $b=1$ then $\kongru{4a^3+ 27b^2}{1}{5} \neq  0 $ and the corresponding elliptic curve $E(\F_5)$ is given by all pairs $(x,y)$ from $\F_5$ such that $y^2=x^3+x+1$. For example if we choose $(x,y)=(1,1)$, we see that $1^2 \neq 1^3+1 + 1$ and hence the pair $(1,1)$ is not an element of our curve $E_1(\F_5)$. On the other hand $(x,y)=(2,1)$ gives $1^2 = 2^3 + 2 + 1$ and hence the pair $(2,1)$ is an element of $E_1(\F_5)$. Doing this computations for all 25 combinations of pairs in $\F_5$, gives the set of rational points as 
$$
E_1(\F_5) = \{\Oinf, (0,1),(2,1),(3,1),(4,2),(4,3),(0,4),(2,4),(3,4)\}
$$
We can also invoke sage to compute this elliptic curve. 
...
\end{sagecommandline}
\end{example}
\begin{example}[Moon-JubJub] Consider the prime field $F_{13}$. If we choose $a=8$ and $b=8$ then $\kongru{4a^3+ 27b^2}{6}{13}$ and the corresponding elliptic curve $MJJ(\F_{13})$ is given by all pairs $(x,y)$ from $\F_13$ such that $y^2=x^3+8x+8$. 

We call $MJJ(\F_{13})$ the Moon-JubJub curve. As we will see in what follows this curve is kind od special as it is possible to represent it in two alternitive forms, called a Montgomery and a twisted Edwards form (See xxx and XXX). 

Since there are $169$ pairs of elements from $F_{13}$, computing the set of curve points is bit to much work and we invoke sage instead. We get
\begin{sagecommandline}
sage: F13 = GF(13)
sage: MJJ = EllipticCurve(F13,[8,8])
sage: AffinePoints = [P.xy() for P in MJJ.points() if P.order() > 1]
\end{sagecommandline}
Combining this with the point at infinity gives the set of rational points as 
\begin{multline*}
...
To give an example consider the elements $P=(0,1)$ and $Q=(4,2)$. Neither of these elements is the neutral element and since the $x$-coordinate of $P$ is different from the $x$-coordinate of $Q$, we know that we have to use the tangent rule, that is rule number 4 from XXX to compute the sum $R= P\oplus Q$. We get
%\begin{tabular}{lr}
\begin{align*}
x_3  & = \left(\frac{y_2-y_1}{x_2-x_1}\right)^2 -x_1-x_2 \\
     & = \left(\frac{2-1}{4-0}\right)^2 -0-4 \\
...
\begin{align*}
y_3  & = \left(\frac{y_2-y_1}{x_2-x_1} \right)\left(x_1-x_3\right) - y_1 \\     
     & = \left(\frac{2-1}{4-1} \right)\left(0-2\right) - 1 \\    
...
\begin{align*}
x'  & = \left(\frac{3x^2+a}{2y}\right)^2 -2x \\
    & = \left(\frac{3\cdot 4^2+1}{2\cdot 2}\right)^2 -2\cdot 4 \\
...
\begin{align*}
y'  & = \left(\frac{3x^2+a}{2y}\right)^2\left(x-x'\right) - y\\
    & = \left(\frac{3\cdot 4^2+1}{2\cdot 2}\right)^2\left(4-3\right) - 2\\
    & = 1\cdot 1 + 3\\
    & 4
\end{align*}
So in our elliptic curve $E_1(\F_5)$ we get the doubling $2\cdot (4,2)$, that is $(4,2)\oplus (4,2) =(3,4)$ and indeed the pair $(3,4)$ is an element of $E_1(\F_5)$ as expected. The group $E_1(\F_5)$ has no self inverse points, since no point has $0$ as its $y$-coordinate.

We can also invoke sage to compute the addition on our  elliptic curve. 
\begin{sagecommandline}
sage: F5 = GF(5)
sage: E1 = EllipticCurve(F5,[1,1])
sage: P = E1(0,1)
...
10), (11, 6), (11, 7), (12, 5), (12, 8)\}
\end{multline*}
In contrast to the group from the previous example, this group contains the self inverse point $(4,0)$. To see that, observe that we can not add $(4,0)$ to itself using the tangent rule 3 from XXX, as the $y$-coordinate is zero. Instead we have to use rule 2, since $0=-0$. We therefore get $(4,0)\oplus (4,0)=\Oinf$ in $MJJ(\F_{13})$. 
\begin{sagecommandline}
sage: F13 = GF(13)
sage: MJJ = EllipticCurve(F13,[8,8])
sage: P = MJJ(4,0)
