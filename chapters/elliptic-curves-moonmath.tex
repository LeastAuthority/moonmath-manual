...
\end{array} 
\end{equation}
of an alliptic curve $E(\F)$ over some finite field $\F$ of characteristic $p$. It can be shown that $\phi$ maps curve points to curve points. The first thing to note is that in case that $\F$ is a prime field, the Frobenious endomorphism acts trivially, since $(x^p,y^p) = (x,y)$ on prime fields, due to Fermat's little theorem XX. So the Frobenious map is more interesting over prime field extensions.
...
\begin{equation}
\G_2[n] := \{(x,y)\in E[n]\;| \phi(x,y) = [p](x,y)\;\}
\end{equation}
...
\paragraph{Pairing} In this part we consider pairing functions defined on certrain subgroups of some full torsion groups of an elliptic curve. To be more precise let 
$E(\F_p)$ be an elliptic curve of order $r$ and embedding degree $k$, let $n$ be a prime factor of $r$, $E[n]$ the full torsion group of the curve and $\G_1$ as well as $\G_2$ two subgroups of $E[n]$. Then an \textbf{elliptic curve pairing} is a bilinear, non-degenerate map
\begin{equation}
e(\cdot,\cdot) : \G_1 \times \G_2 \to \F_{p^k}
\end{equation} 
in the sense of our definition XXX. In practise, the embedding degree is usuall a small number like $2$, $4$, $6$ or $12$ and the prime factor $n$ is the largest prime factor of the curves order. In addition $\G_1$ is usually choosen to be the $n$-torsion group of the curve $E(\F_p)$ over the base field.
...
There is however a more precise estimation, usually called the \textbf{Hasse bound}. To understand it, let $E(\F)$ be an affine short Weierstraß curve over a finite field $(\F)$ of order $q$ and let $|E(\F)|$ be the number of elements in that curve. Then there is an integer $t\in \Z$ called the \textbf{trace} of the curve, such that $|t| \leq 2\sqrt{q}$ and
\begin{equation}
|E(\F)| = q +1 -t
\end{equation}
