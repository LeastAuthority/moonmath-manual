...
\end{example}
\begin{example}[The Tiny-Jubjub Curve] Consider the language $L_{tiny.jj}$ from example \ref{ex:tiny-jubjub}. As we have seen, a constructive proof in $L_{tiny.jj}$ is given by a pair $(x_1,x_2)$ of field elements from $\F_{13}$ such that the pair is a point of the tiny-jubjub curve in its Edwards representation.
...
\end{itemize}
\end{exercise}
\begin{exercise}[Edwards Addition on Tiny-Jubjub] Consider the tiny-jubjub curve together with its twisted Edwards addition law from example \ref{TJJ13}. Define an instance alphabet $\Sigma_I$, a witness alphabet $\Sigma_W$ and a decision function $R_{add}$ with associated language $L_{add}$ such that a string $(i\,;w)\in \Sigma_I^* \times \Sigma_W^*$ is a word in $L_{add}$ if and only if $i$ is a pair of curve points on the tiny-jubjub curve in Edwards form and $w$ is the sum of those curve points.
...
\label{sec:R1CS}
Although decision functions are expressible in various ways, many contemporary proving systems require the deciding relation to be expressed in terms of a system of quadratic equations over a finite field. This is true in particular for pairing-based proving systems like XXX\sme{add reference}, roughly because it is possible to check solutions to those equations ``in the exponent'' of pairing-friendly cryptographic groups.
...
\end{align*}
\end{definition}
If a rank-1 constraint system is given, the parameter $k$ is called the \term{number of constraints}  If a string $(I_1,\ldots, I_n; W_1,\ldots,W_m)$ of field elements satisfies theses equations, $(I_1,\ldots, I_n)$ is called an \term{instance} and $(W_1,\ldots,W_m)$ is called an associated \term{witness} of the system.
...
\end{example}
\begin{example} To give a more realistic example of an algebraic circuit, look at the defining equation  of the tiny-jubjub curve (\ref{TJJ13}\sme{check reference}) again. A pair of field elements 
