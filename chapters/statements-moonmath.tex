...
The purpose of this chapter is to explore methods of formalizing statements in a mathematically rigorous way that is useful for SNARK development. To accomplish this, we introduce formal languages as a means of properly defining statements (\secname{} \ref{sec:formal-languages}). For a more detailed explanation of formal languages, see \cite{moll-2012}. Additionally, we examine algebraic circuits and rank-1 constraint systems (R1CS) as two particularly valuable ways of defining statements within specific formal languages. The introduction to R1CS and algebraic circuits can be found in appendix E of \cite{sasson-2013}.

Given that unintended true statements can result in potentially severe and difficult-to-detect security vulnerabilities in SNARK applications, proper statement design should be a top priority in the development of SNARKs.
...
Some words about turbo Plonk ect, and this is classic Plonk
\subsection{PLONK Constraint Systems}
In order to gain a detailed understanding of PLONK's Constraint Systems (PCS), we assume the existence of a field $\F$, a natural number $k$, "binary" elements $l_j, r_j, m_j, o_j\in\{0,1\}\subset\F$ and constants $c_j\in \F$ for every index $1\leq j \leq k$ and a permutation
\begin{equation}
\sigma: \langle 1,\ldots , 3k \rangle \to \langle 1,\ldots , 3k \rangle
\end{equation}
Given these parameters, a PLONK constraint system can be defined as the following set of equations:
\begin{equation}
\label{Plonk-CS}
\begin{array}{ll}
\text{Gate Constraints}: &
\begin{cases}
\; l_1 \cdot S_{1} + r_1\cdot S_{k+1} + m_1 \cdot S_{1} \cdot S_{k+1} = o_1 \cdot S_{2k+1} + c_1 \\
\; l_2 \cdot S_{2} + r_2\cdot S_{k+2} + m_2 \cdot S_{2} \cdot S_{k+2} = o_2 \cdot S_{2k+2} + c_2 \\
\;\vdots \\
\; l_k \cdot S_{k} + r_k\cdot S_{k+k} + m_k \cdot S_{k} \cdot S_{k+k} = o_k \cdot S_{2k+k} + c_k
\end{cases}\\
...
\end{array}
\end{equation}
If there exists a pair of strings of field elements $\langle S_1,\ldots S_{3k} \rangle = (\langle I_1, \ldots I_n \rangle;\langle W_{n+1}, \ldots W_{3k}\rangle)$ that satisfies  equations \ref{Plonk-CS} for some $n\leq k$, then $<I_1,\ldots, I_n>$ is denoted as an \term{instance} and $<W_{n+1},\ldots,W_{3k}>$ is referred to as a \term{witness} of the system.

The \term{copy constraints} are defined by the permutation $\sigma$, while the field elements $c_j$ are \term{system constants} that are known at compile time. The constants $l_j$, $r_j$, $m_j$, and $o_j$ are (boolean) \term{selector constants}, and the parameter $n$ represents the number of \term{public inputs} while $k$ denotes the number of \term{gate constraints}.

\concept{PLONK constraint systems} provide a universal model for bounded computations, as they can express any computation that is guaranteed to terminate in a finite number of steps.

The essential idea behind a \concept{PLONK constraint system} is to capture all the possible values that each variable can take on during a computation and to formalize the relationships between those variables that are implied by the computation itself. By doing so, the execution of a computer program is enforced to occur exactly as expected, leaving no room for deviations. In this way, solutions to \concept{PLONK constraint systems} serve as proofs of proper program execution.

\begin{example}[PLONK for 3-Factorization]\label{ex:3-factorization-plonk} To provide a better intuition of \concept{PLONK constraint systems}, consider the language $L_{3.fac\_zk}$ from \examplename{} \ref{ex:L-3fac-zk} again. As we have seen, $L_{3.fac\_zk}$ consists of words $(<I_1>;<W_1,W_2,W_3>)$ over the alphabet $\F_{13}$ such that $I_1 = W_1\cdot W_2\cdot W_3$. We show how to rewrite the problem as a \concept{PLONK constraint system}.

Since PCS are systems of quadratic equations, expressions like $W_1\cdot W_2\cdot W_3$, which contain products of more than two factors (which are therefore not quadratic) have to be rewritten in a process often called \term{flattening}. To flatten the defining equation $I_1 = W_1\cdot W_2\cdot W_3$ of $L_{3.fac\_zk}$, we first relabel indices and write the defining equation as $I_1 = W_2\cdot W_3\cdot W_4$. Since the multiplication on the right can be expressed by two multiplications, we can write it as follows:
\begin{align*}
W_{2} \cdot W_{3} & = W_{4} & \text{gate constraint } 1 \\
W_{4} \cdot W_{5} & = W_{6} & \text{gate constraint } 2 \\
\\
W_{} = W_{(2,3)} & & \text{equality constraint } 1 \\
\end{align*}
Given some instance $I_1$, any solution $(W_1,W_2,W_3,W_4)$ to this system of equations provides a solution to the original equation $I_1 = W_1\cdot W_2\cdot W_3$ and vice versa. Both equations are therefore equivalent in the sense that solutions are in a 1:1 correspondence.
...
\; l_1 \cdot S_{1} + r_1\cdot S_{k+1} + m_1 \cdot S_{1} \cdot S_{k+1} = o_1 \cdot S_{2k+1} + c_1 \\
\; l_2 \cdot S_{2} + r_2\cdot S_{k+2} + m_2 \cdot S_{2} \cdot S_{k+2} = o_2 \cdot S_{2k+2} + c_2 \\
\; l_3 \cdot S_{3} + r_3\cdot S_{3+3} + m_3 \cdot S_{3} \cdot S_{3+3} = o_3 \cdot S_{2\cdot 3+3} + c_3
\end{cases}\\
\text{Equality Constraints}: &
\begin{cases}
\; S_{1} = S_{\sigma(1)}\\
