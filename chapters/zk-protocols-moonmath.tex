...
A so-called \term{zero-knowledge protocol} is a set of mathematical rules by which one party (usually called \term{the prover}) can convince another party (usually called \term{the verifier}) that a given statement is true, while not revealing any additional information apart from the truth of the statement. 

As we have seen in chapter \ref{chap:statements}\sme{add reference}, given some language $L$ and instance $I$, the knowledge claim ``there is a witness $W$ such that  $(I;W)$ is a word in $L$'' is constructively provable by providing $W$ to the verifier. However, the challenge for a zero-knowledge protocol is to prove knowledge of a witness without revealing any information beyond its bare existence.
...
A proof system is usually called \term{succinct} if the size of the proof is shorter than the witness necessary to generate the proof. Moreover, a proof system is called \term{computationally sound} if soundness only holds under the assumption that the computational capabilities of the prover are polynomially bound. To distinguish general proofs from computationally sound proofs, the latter are often called \term{arguments}. Zero-knowledge, succinct, non-interactive arguments of knowledge claims are often abbreviated \term{zk-SNARKs}.
\begin{example}[Constructive Proofs for Algebraic Circuits] To formalize our previous notion of constructive proofs for algebraic circuits, let $\F$ be a finite field, and let $C(\F)$ be an algebraic circuit over $\F$ with associated language $L_{C(\F)}$. A non-interactive proof system for $L_{C(\F)}$ is given by the following two algorithms:
...
The generated argument is perfectly zero-knowledge has perfect completeness and soundness in the generic bilinear group model, assuming the existence of a trusted third party that executes a preprocessing phase to generate a \uterm{common reference string} and a \uterm{simulation trapdoor}. This party must be trusted to delete the simulation trapdoor, since everyone in possession of it can simulate proofs. 
...
\begin{example}[The 3-Factorization Problem] Consider the $3$-factorization problem from \ref{ex:3-factorization}\sme{check reference} and its associated algebraic circuit and rank-1 constraint system from  \ref{C3fac}\sme{check reference}. In this example, we want to agree on a parameter set $(R,r, \G_1, \G_2, e(\cdot,\cdot), g_1,g_2)$ in order to use the Groth\_16 protocol for our $3$-factorization problem. 

To find proper parameters, first observe that the circuit XXX\sme{add reference}, as well as its associated R1CS $R_{3.fac\_zk}$ {ex:3-factorization-r1cs}\sme{check reference} and the derived QAP \ref{QAP-R3-fac-zk}\sme{check reference}, are defined over the field $\F_{13}$. We therefore have $r=13$ and need pairing groups $\G_1$ and $\G_2$ of order $13$. 

We know from \ref{BLS6} that the moon-math curve \texttt{BLS6\_6} has two subgroups $\G_1[13]$ and $\G_2[13]$, which are both of order $13$. The associated Weil pairing $b$ (\ref{BLS6-weil-pairing})\sme{check reference} is a proper bilinear map. We therefore choose those groups and the Weil pairing together with the generators $g_1 = (13,15) $ and $g_2=(7v^2,16v^3)$ of $\G_1[13]$ and $\G_2[13]$, as a parameter:
$$
...
The setup phase then samples $5$ random, \smelong{inverible}\sme{"invariable"?} elements $\alpha$, $\beta$,$\gamma$, $\delta$ and $s$ from the scalar field $\F_r$ of the protocol and outputs the \term{simulation trapdoor} $\tau$:
\begin{equation}
\tau = (\alpha, \beta, \gamma, \delta, s)
\end{equation}
In addition, the setup phase uses those $5$ random elements together with the two generators $g_1$ and $g_2$  and the quadratic arithmetic program to generate a \term{common reference string} $CRS_{QAP}= (CRS_{\mathbb{G}_1},CRS_{\mathbb{G}_2})$ of language $L$:
\begin{align*}
...
In real-world applications, the simulation trapdoor is often called the \term{toxic waste}\sme{explain why} of the setup-phase, while a common reference string is also-called the pair of \term{prover and verifier key}. 

In order to make the protocol secure, the setup needs to be executed in a way that  guarantees that the simulation trapdoor is deleted. Anyone in possession of it can generate arguments without knowledge of a constructive proof. The most simple approach to achieving deletion of the toxic waste is by a so-called \term{trusted third party}, where the trust assumption is that that the party generates the common reference string precisely as defined and deletes the simulation backdoor afterwards.
...
\begin{example}[The $3$-factorization Problem]\sme{4 examples have the same title. Change it to be distinct}
To see how the setup phase of a Groth\_16 zk-SNARK can be computed, consider the $3$-factorization problem from \ref{ex:3-factorization}\sme{check reference} and the parameters from page \pageref{groth16-parameters}\sme{add reference}. As we have seen in \ref{QAP-R3-fac-zk}\sme{check reference}, an associated quadratic arithmetic program is given as follows:
\begin{multline*}
...
We keep this secret in order to simulate proofs later on, butwe are careful though to hide $\tau$ from anyone who hasn't read this book. Then we instantiate the common reference string XXX\sme{add reference} from those values. Since our groups are subgroups of the \texttt{BLS6\_6} elliptic curve, we use scalar product notation instead of exponentiation. 

To compute the $\G_1$ part of the common reference string, we use the logarithmic order of the group $\G_1$ XXX\sme{add reference}, the generator $g_1=(13,15)$, as well as the values from the simulation backdoor. Since $deg(T)=2$, we get the following:
\begin{align*}
...
\end{array}\right\}
$$
To compute the $\G_2$ part of the common reference string, we use the logarithmic order of the group $\G_2$ XXX\sme{add reference}, the generator $g_2=(7v^2,16v^3)$, as well as the values from the simulation backdoor. Since $deg(T)=2$, we get the following:
\begin{align*}
...
$$
Given the simluation trapdoor $\tau$ and the quadratic arithmetic program \ref{QAP-R3-fac-zk}\sme{check reference}, the associated common reference string of the $3$-factorization problem is as follows:
\begin{align*}
CRS_{\mathbb{G}_{1}} &=\left\{ \begin{array}{c}
(27,34),(26,34),(38,15),\Big((13,15),(33,34)\Big),
\Big(\mathcal{O}, (33,9)\Big)\\
...
To see that, consider e.g. the polynomials $A_2(x)= 6x +10$ and $A_5(x)=7x+4$ from the QAP of this problem. To evaluate these polynomials in the exponent of $g_1$ and $g_2$ at the secret point $s$ without knowing the value of $s$ (which is $2$), we can use the common reference string and equation XXX\sme{add reference}. Using the scalar product notation instead of exponentiation, we get the following:
\begin{align*}
...
In order to generate a zk-SNARK for this instance, the prover first computes a valid constructive proof as explained in XXX\sme{add reference}, that is, the prover generates a proper witness $W= (W_1,\ldots,W_m)$ such that  $(I_1,\ldots, I_n; W_1,\ldots,W_m)$ is a solution to the rank-1 constraint system $R$. 

The prover then uses the quadratic arithmetic program and computes the polynomial $P_{(I;W)}$, as explained in \ref{polynomial-P-IW}\sme{check reference}. They then divide $P_{(I;W)}$ by the target polynomial $T$ of the quadratic arithmetic program. Since $P_{(I;W)}$ is constructed from a valid solution to the R1CS, we know from \ref{polynomial-P-IW}\sme{check reference} that it is divisible by $T$. This implies that polynomial division of $P$ by $T$ generates another polynomial $H:= P / T$, with $deg(H)<deg(T)$. 

The prover then evaluates the polynomial $(H\cdot T)\delta^{-1}$ in the exponent of the generator $g_1$ at the secret point $s$, as explained in XXX\sme{add reference}. To see how this can be achieved, let $H(x)$ be the quotient polynomial $P/T$:
...
a zk-SNARK, consider the $3$-factorization problem from \ref{ex:3-factorization}\sme{check reference}, our protocol parameters from XXX\sme{add reference} as well as the common reference string from XXX\sme{add reference}.

Our task is to compute a zk-SNARK for the instance $I_1=11$ and its constructive proof $(W_1,W_2,W_3,W_4)=(2,3,4,6)$ as computed in XXX\sme{add reference}. As we know from \ref{polynomial-P-IW},\sme{check reference} the associated polynomial $P_{(I;W)}$ of the quadratic arithmetic program from XXX\sme{add reference} is given by the following equation:
$$
P_{(I;W)} = x^2 + x + 9
$$ 
Since $P_{(I;W)}$ is identical to the target polynomial $T(x)=x^2+x + 9$ in this example, we know from XXX\sme{add reference} that the quotient polynomial $H=P/T$ is the constant degree $0$ polynomial:
$$
H(x)= H_0 \cdot x^0 = 1 \cdot x^0
$$
We therefore use $[\frac{s^0\cdot T(s)}{\delta}]g_1=(26,34)$ from our common reference string XXX\sme{add reference} of the $3$-factorization problem and compute as follows:
\begin{align*}
...
To compute this point, we have to remember that a prover should not be in possession of the simulation trapdoor, hence, they do not know what $\alpha$, $\beta$, $\delta$ and $s$ are. In order to compute this group element, the prover therefore needs the common reference string. Using the logarithmic order from XXX\sme{add reference} and the witness, we get the following:
...
Since we don't know what $\alpha$, $\delta$ and $s$ are, we look up $[\alpha]g_1$ and $[\delta]g_1$ from the common reference string. Recall from XXX\sme{add reference} that we can evaluate $[A_j(s)]g_1$ without knowling the secret evaluation point $s$. According to XXX\sme{add reference}, we have $[A_2(s)]g_1=(35,15)$, $[A_5(s)]g_1=(26,34)$ and $[A_j(s)]g_1=\mathcal{O}$ for all other indices $0\leq j\leq 5$. Since $\mathcal{O}$ is the neutral element on $\G_1$, we get the following:
\begin{align*}
...
Since we don't know what $\beta$, $\delta$ and $s$ are, we look up the associated group elements from the common reference string. Recall from XXX\sme{add reference} that we can evaluate $[B_j(s)]g_1$ without knowing the secret evaluation point $s$. Since $B_3=A_2$ and $B_4=A_5$, we have $[B_3(s)]g_1=(35,15)$, $[B_4(s)]g_1=(26,34)$ according to XXX\sme{add reference}, and $[B_j(s)]g_1=\mathcal{O}$ for all other indices $0\leq j\leq 5$. Since $\mathcal{O}$ is the neutral element on $\G_1$, we get the following:
\begin{align*}
...
a zk-SNARK for some given instance $I$, consider the $3$-factorization problem from \ref{ex:3-factorization}\sme{check reference}, our protocol parameters from XXX\sme{add reference}, the common reference string from XXX\sme{add reference} as well as the zk-SNARK $\pi=((35,15),(27,9),(7v^2,27v^3))$, which claims to be an argument of knowledge for a witness for the instance $I_1=11$.
...
To compute this point, we have to remember that a verifier should not be in possession of the simulation trapdoor, which means that they do not know what $\alpha$, $\beta$, $\gamma$ and $s$ are. In order to compute this group element, the verifier therefore needs the common reference string. Using the logarithmic order from XXX\sme{add reference} and instance $I_1$, we get the following:
...
In the next step, we have to compute all the pairings involved in equation XXX\sme{add reference}. Using the logarithmic order on $\G_1$ and $\G_2$ as well as the bilinearity property of the pairing map we get the following:
...
In order to check equation XXX\sme{add reference}, observe that the target group $\G_T$ of the Weil pairing is a finite cyclic group of order $13$. Exponentiation is therefore done in modular $13$ arithmetics. Accordingly, since $\Zmod{108}{13}=4$, we evaluate the left side of equation XXX\sme{add reference} as follows:

$$
e([A]g_1,[B]g_2) = e((13,15),(7v^2,16v^3))^{108} =e((13,15),(7v^2,16v^3))^{4}
$$

Similarly, we evaluate the right side of equation XXX\sme{add reference} using modular $13$ arithmetics and the exponential law $a^x\cdot a^y = a^{x+y}$:
...
As we can see, both the left and the right side of equation XXX\sme{add reference} are identical, which implies that the verification process accepts the simulated proof.

\smelong{NOTE: UNFORTUNATELY NOT! :-(( HENCE THERE IS AN ERROR SOMEWHERE ... NEED TO FIX IT AFTER VACATION}\sme{fix error}

\end{example}

\paragraph{Proof Simulation} During the execution of a setup phase, a common reference string is generated, along with a simulation trapdoor, the latter of which must be deleted at the end of the setup-phase. As an alternative, a more complicated multi-party protocol like [XXX\sme{add reference}] can be used to split the knowledge of the simulation trapdoor among many different parties.
...
zk-SNARK for some given instance $I$, consider the $3$-factorization problem from  \ref{ex:3-factorization}\sme{check reference}, our protocol parameters from XXX\sme{add reference}, the common reference string from XXX\sme{add reference} and the simulation trapdoor $\tau = (6,5,4,3,2)$ of that CRS.

In order to forge a zk-SNARK for instance $I_1=11$, we don't need a constructive proof for the associated rank-1 constraint system, which implies that we don't have to execute the circuit $C_{3.fac}(\F_{13})$. Instead, we have to choose $2$ arbitrary elements $A$ and $B$ from $\F_{13}$, and compute $g_1^A$, $g_2^B$ and $g_1^C$ as defined in XXX\sme{add reference}. We choose $A=9$ and $B=3$, and, since $\delta^{-1}=3$, we compute as follows:
...
To see that, we show that our forged SNARK passes the verification process. In order to verify $\pi_{fake}$, we proceed as in XXX,\sme{add reference} and compute the curve point $g_1^I$ for the instance $I_1=11$. Since the instance is the same as in example XXX\sme{add reference}, we can parallel the computation from XXX\sme{add reference}:

\begin{align*}
[I]g_1 & = [\frac{\beta\cdot A_{0}(s)+\alpha\cdot B_{0}(s)+C_{0}(s)}{\gamma}]g_1 \oplus [I_1][\frac{\beta\cdot A_{1}(s)+\alpha\cdot B_{1}(s)+C_{1}(s)}{\gamma}]g_1\\
       & = (35,28)
\end{align*} 
In a next step we have to compute all the pairings involved in equation XXX\sme{add reference}. Using the logarithmic order on $\G_1$ and $\G_2$ as well as the bilinearity property of the pairing map we get
\begin{align*}
...
In order to check equation XXX\sme{add reference}, observe that the target group $\G_T$ of the Weil pairing is a finite cyclic group of order $13$. Exponentiation is therefore done in modular $13$ arithmetics. Using this, we evaluate the left side of equation XXX\sme{add reference} as follows:
$$
e([A]g_1,[B]g_2) = e((13,15),(7v^2,16v^3))^{27} =e((13,15),(7v^2,16v^3))^{1}
$$
since $\Zmod{27}{13}=1$. Similarly, we evaluate the right side of equation XXX\sme{add reference} using modular $13$ arithmetics and the exponential law $a^x\cdot a^y = a^{x+y}$. We get
\begin{align*}
e([\alpha]g_1,[\beta]g_2)\cdot e([I]g_1,[\gamma]g_2)\cdot e([C]g_1,[\delta]g_2) =\\
