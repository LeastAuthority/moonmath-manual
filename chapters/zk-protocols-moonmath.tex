...
The prover then uses the quadratic arithmetic program and computes the polynomial $P_{(I;W)}$, as explained in \ref{polynomial-P-IW}\sme{check reference}. They then divide $P_{(I;W)}$ by the target polynomial $T$ of the quadratic arithmetic. Since $P_{(I;W)}$ is constructed from a valid solution to the R1CS, we know from \ref{polynomial-P-IW}\sme{check reference} that it is divisible by $T$. This implies that polynomial division of $P$ by $T$ generates another polynomial $H:= P / T$, with $deg(H)<deg(T)$. 
