...
\begin{equation}
\label{groth16-parameters}
\mathtt{Plonk-Param}(k)=(r, \G_1, \G_2, e(\cdot,\cdot), g_1,g_2, H)
\end{equation}

Here, $\G_1$ and $\G_2$ are finite cyclic groups of order $r$ and $H:=\{1,\omega,\omega^2,\ldots, \omega^{k-1}\}$, where $\omega := \sqrt[k]{1}$ is the $k$-th root of unity in $\F_r$.
Moreover $g_1$ is a generator of $\G_1$, $g_2$ is a generator of $\G_2$ and $e: \G_1 \times \G_2 \to \G_T$ is an efficiently computable, non-degenerate, bilinear pairing for some target group $\G_T$. 


In real-world applications, the parameter set is usually agreed on in advance. 
