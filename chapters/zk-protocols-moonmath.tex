...
To be more precise, let $L_R$ be a language defined by some Plonk Constraint System $R$ of size $n$, such that $l_i$, $r_i$, $m_i$, $o_i$ and $c_i$ are the selector constants of the system for any $1,\ldots, m$ and such that $\sigma$ is is the copy constraints permutation and let $\{\G_1, \G_2, e(\cdot,\cdot), g_1, g_2, \F_r, H, \omega, k_1, k_2, L_H\}$ be a set of Plonk parameters.

The second setup phase then computes the following set of polynomials, where the polynomials $L_i$ are the Lagrange base polynomials as computed in the universal setup phase XXX.
\begin{equation}
\begin{array}{rl}
Z_{H}(x) = & \Pi_{i=0}^n (x-\omega^i)= x^n-1\\
...
samples $5$ random, invertible elements $\alpha$, $\beta$,$\gamma$, $\delta$ and $\tau$ from the scalar field $\F_r$ of the protocol and outputs the \term{simulation trapdoor} $\Tau$:
\begin{equation}
\label{def:groth16-trapdoor}
\Tau = (\alpha, \beta, \gamma, \delta, \tau)
\end{equation}
...
In the Plonk protocol, the circuit specific setup phase does not require a trusted third party and depends on the Universal Common Reference String as well as the permutation $\sigma$ and the selector constants $l_i$, $r_i$, $m_i$, $o_i$ and $c_i$ of a given Plonk constraint system (See XXX).    
\begin{equation}
\begin{array}{rl}
Z_{H}(x) = & \Pi_{i=0}^n (x-\omega^i)= x^n-1\\
P_{m}(x) = &\sum_{i=1}^{n} {m_i}L_i(x)\\
...
\begin{align*}
P_{m}(x) = & \sum_{i=1}^{n} {m_i}L_i(x)\\
         = & m_0 10x^3 + 10 x^2 +10x + 10, 11x^3 +3x^2 +2x + 10, \\ 3x^3 +10x^2+ 3x + 10, 2x^3 + 3x^2 +11x +10
\end{align*}
