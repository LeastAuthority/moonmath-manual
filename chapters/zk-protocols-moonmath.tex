...
\begin{align} \label{eq:3-fac-A2-A5-g1}
...
\begin{align} \label{eq:3-fac-A2-A5-g2}
...
\begin{align}\label{eq:groth16_prover}
...
In a next step, we compute $g_1^A$ (see eq. \eqref{eq:groth16_prover}). We sample the random point $r=11$ from $\F_{13}$, using scalar products instead of the exponential notation, and $\oplus$ for the group law on the \texttt{BLS6\_6} curve. We then have to compute the following expression:
...
In order to compute the two curve points $[B]g_1$ and $[B]g_2$ (see eq. \eqref{eq:groth16_prover}), we sample another random element $t=4$ from $\F_{13}$. Using the scalar product instead of the exponential notation, and $\oplus$ for the group law on the \texttt{BLS6\_6} curve, we have to compute the following expressions:
...
In a last step, we combine the previous computations to compute the point $[C]g_1$ (see eq. \eqref{eq:groth16_prover}) in the group $\G_1$ as follows:
\begin{align*}
