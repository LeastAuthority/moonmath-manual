...
In order to verify the zk-SNARK for that instance, we first compute the curve point $g_1^I$ (see eq. \eqref{eq:groth16_verify}). Using scalar products instead of the exponential notation, and $\oplus$ for the group law on the \texttt{BLS6\_6} curve, we have to compute the point $[I]g_1$ as follows:
