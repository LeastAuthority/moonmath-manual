...
\label{sec:PLONK}
The \term{Plonk} protocol, developed by Ariel Gabizon, Zachary Williamson, and Oana Ciobotaru in \cite{gabizon-19}, provides a method for transforming constructive proofs for formal languages into zero-knowledge succinct non-interactive arguments of knowledge. 

Plonk, at its core, comprises two distinct elements: firstly, an arithmetization process, which transforms arbitrary bounded computations into  a series of polynomial equations; and secondly, a polynomial commitment scheme, enabling a verifier to succinctly validate these equations, while achieving varying degrees of zero-knowledge assurance.
...
\subsection{The Plonk Parameters}
Looking more closely at the original Plonk protocol, let $m\in \N$ be a natural number that defines an upper bound on the maximal number of constraints that the system can handle. Then \term{Plonk parameters} for $m$ are given by the following set:
\begin{equation}
...
\begin{itemize}
\item (Universal Setup-Phase): $(CRS(m),\Tau)\leftarrow \textsc{U-Setup}(m)$: Algorithm $\textsc{U-Setup}$ takes an upper bound on the number of constraints as input and computes a \term{\concept{Universal Common Reference String}} $CRS$ for the bound $m$ and a \term{simulation trapdoor} $\Tau$.
...
\end{equation}
Note that Universal Common Reference Strings are not unique to a specific problem since they depend on the simulation trapdoor. A Plonk Constraint System may have more than one Universal Common Reference String. If a simulation trapdoor $\Tau = (\tau)$ is provided, then $\tau$ is referred to as a \term{secret evaluation point} in the protocol. This is because a key property of a \concept{Universal Common Reference String} is that it enables the evaluation of any polynomial $P\in \F_r[x]$ of degree $deg(P)<m+5$ at the point $\tau$ in the exponent of the generator $g_1$, without revealing the actual value of $\tau$. In this sense the polynomial is evaluated at $\tau$ but without actually knowing $\tau$.

To be more precise, suppose that $\tau$ is the secret evaluation point and let $P(x)=a_0\cdot x^0 + a_1\cdot x^1 + \ldots a_k\cdot x^k$ be a polynomial of degree $k<m+5$ with coefficients in $\F_r$. Then we can use \ref{def:plonk-ucrs} to compute $g_1^{P(\tau)}$ without disclosing the value of $\tau$:
\begin{align}
...
For efficiency reasons it is often common practice, to extend the UCRS \ref{def:plonk-ucrs} by all circuit independent data that prover and verifier need to execute the protocol. In particular it is common practice to precompute the Lagrange base XXX $L_H= \{L_1,\ldots,L_m\}$ and the vanishing polynomial $Z_H$ of the $m$-th roots of unity group $H$ as well as their evaluation $L'_H= \{g_1^{L_1},\ldots,g_1^{L_m}\}$ and $g_1^{Z_H}$ in the exponent of $g_1$. This data is then usually added to the original UCRS.   
\begin{example}[The $3$-factorization Problem]
\label{ex:3-fac-UCRS}
...
\Tau = (2)
$$
We keep the simulation trapdoor secret in order to simulate proofs later on. Then we instantiate the \concept{Universal Common Reference String} \ref{def:plonk-ucrs} from this value, by computing the powers of tau on the generator $g_1$. Since $\G_1$ is a subgroup of the \texttt{BLS6\_6} elliptic curve, we use scalar product notation instead of exponentiation (See XXX). 
...
\begin{sagecommandline}
sage: F43 = GF(43)
sage: BLS6 = EllipticCurve(F43, [0,6])
sage: g1=BLS6(13,15)
sage: UCRS = []
...
\begin{sagecommandline}
sage: F13 = GF(13)
sage: F13t.<t> = F13[] # Polynomial Ring over F13
...
sage: L1 = F13t.lagrange_polynomial(S1)
sage: L2 = F13t.lagrange_polynomial(S2)
sage: L3 = F13t.lagrange_polynomial(S3)
sage: L4 = F13t.lagrange_polynomial(S4)
sage: ZH = F13t((t-5)*(t-12)*(t-8)*(t-1))
sage: LH = [L1,L2,L3,L4]
sage: LH, ZH
\end{sagecommandline} 
Assuming that we have eliminated the simulation trapdoor, let's see how the \concept{Universal Common Reference String} can aid in evaluating the Lagrange base polynomials at the secret evaluation point within the generator exponent. Without the simulation trapdoor, and provided that our group is DL-secure XX, we no longer have access to the secret evaluation point. As a result, we are unable to evaluate polynomials at that point. Nonetheless, we can still evaluate polynomials with a degree less than $m+5$ within the exponent of the generator at that point. Using the \concept{Universal Common Reference String} in combination with equation XXX, we obtain the following:
\begin{align*}
...
[\tau\cdot Z_H(\tau)]g_1 & = (27,34) \oplus [12](33,34)\\
               ...
               & = (35,28)              
\end{align*}
We can use sageimagi
\begin{sagecommandline}
sage: def scalar_sum(coef, base):
....:     result = BLS6(0)
....:     for i in range(len(coef)):
....:         result += ZZ(coef[i]) * base[i]
....:     return result
sage: gL1 = scalar_sum(L1.coefficients(sparse=False), UCRS)
sage: gL2 = scalar_sum(L2.coefficients(sparse=False), UCRS)
sage: gL3 = scalar_sum(L3.coefficients(sparse=False), UCRS)
sage: gL4 = scalar_sum(L4.coefficients(sparse=False), UCRS)
sage: gZH = scalar_sum(ZH.coefficients(sparse=False), UCRS)
sage: tZH = ZH*t
sage: gtZH = scalar_sum(tZH.coefficients(sparse=False), UCRS)
sage: gLH = [gL1, gL2, gL3, gL4]
sage: gLH, gZH, gtZH
\end{sagecommandline}
\end{example}
...
To be more specific, let $\mathtt{Plonk-Param}(m)=(r, \G_1, \G_2, e(\cdot,\cdot), g_1,g_2, H, k_1,k_2, \mathtt{Sponge})$ be a set of Plonk parameters, $UCRS(m)$ a Universal Common Reference String of size $m$, $L=\{L_1,\cdot,L_m\}$ a Lagrange base for the set $H$ and let $R$ denote the following Plonk Constraint System of size $n\leq m$ with copy constraint permutation $\sigma$:
\begin{table}[H]
\centering
\begin{tabular}{|c|ccc|cccc|c|ccc|}
\hline
$\mathbb{H}$ & $x_l$ & $x_r$ & $x_o$ & $s_l$ & $s_r$ & $s_m$ & $s_o$ & $c$ & \multicolumn{3}{c|}{$\sigma(\;x_l,\;\; x_r,\;\; x_o\;)$} \\ \hline
$\omega^1$ & $x_l^{(1)}$ & $x_r^{(1)}$ & $x_o^{(1)}$ & $s_l^{(1)}$ & $s_r^{(1)}$ & $s_m^{(1)}$ & $s_o^{(1)}$ & $c^{(1)}$ & $\sigma(1)$ & $\sigma(n+1)$ & $\sigma(2n+1)$ \\
$\omega^2$ & $x_l^{(2)}$ & $x_r^{(2)}$ & $x_o^{(2)}$ & $s_l^{(2)}$ & $s_r^{(2)}$ & $s_m^{(2)}$ & $s_o^{(2)}$ & $c^{(2)}$ & $\sigma(2)$ & $\sigma(n+2)$ & $\sigma(2n+2)$ \\
$\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ \\
$\omega^n$ & $x_l^{(n)}$ & $x_r^{(n)}$ & $x_o^{(n)}$ & $s_l^{(n)}$ & $s_r^{(n)}$ & $s_m^{(n)}$ & $s_o^{(n)}$ & $c^{(n)}$ & $\sigma(n)$ & $\sigma(n+n)$ & $\sigma(2n+n)$ \\ \hline
\end{tabular}
\end{table}


The second setup phase then computes the following constraint specific \term{Prover key}, which consists of the following set of polynomials.
\begin{equation*}
...
\end{array}
\end{equation*}
In addition the group elements from the Universal Common Reference String can be used to evaluate these polynomials in the exponent of the generator $g_1$, which gives the following \term{verifier key}:
\begin{equation}
...
\hline
$\mathbb{H}$ & $x_l$ & $x_r$ & $x_o$ & $s_l$ & $s_r$ & $s_m$ & $s_o$ & $c$ & \multicolumn{3}{c|}{$\sigma(\;x_l,\;\; x_r,\;\; x_o\;)$} \\ \hline
$5$ & $x_l^{(1)}$ & $x_r^{(1)}$ & $x_o^{(1)}$ & $0$ & $0$ & $1$ & $1$ & $0$ & $1$ & $5$ & $2$ \\
$12$ & $x_l^{(2)}$ & $x_r^{(2)}$ & $x_o^{(2)}$ & $0$ & $0$ & $1$ & $1$ & $0$ & $9$ & $6$ & $3$ \\
$8$ & $x_l^{(3)}$ & $x_r^{(3)}$ & $x_o^{(3)}$ & $1$ & $0$ & $0$ & $0$ & $I$ & $10$ & $7$ & $11$ \\
$1$ & $x_l^{(4)}$ & $x_r^{(4)}$ & $x_o^{(4)}$ & $0$ & $0$ & $0$ & $0$ & $0$ & $4$ & $8$ & $12$ \\ \hline
\end{tabular}
\end{table}
...
(27,34), (13,28), (33,9), (35,15), (26,34), (38,28)\}
\end{multline*}
Computing the selector polynomials we first observe that since compiling the 3-fac circuit into a Plonk constraints system only requires 3 constraints XXX, all selector entries in the forth row are zero, which implies that we can exclude that summand from the computation of those polynomials. Using the Lagrange base $L_H$ from the universal setup phase XXX, we get:
$$
\begin{array}{rl}
P_{l}(x) = & \sum_{i=1}^{n} s_l^{(i)}L_i(x)\\         
         = & 0\cdot(11x^3 +3x^2 +2x + 10) + 0\cdot (3x^3 +10x^2+ 3x + 10) + 1\cdot (2x^3 + 3x^2 +11x +10) \\ 
         = & 2x^3 + 3x^2 +11x +10 \\
P_{r}(x) = & \sum_{i=1}^{n} s_r^{(i)}L_i(x)\\         
         = & 0\cdot(11x^3 +3x^2 +2x + 10) + 0\cdot (3x^3 +10x^2+ 3x + 10) + 0\cdot (2x^3 + 3x^2 +11x +10) \\ 
         = & 0 \\
P_{s_m}(x) = & \sum_{i=1}^{n} s_m^{(i)}L_i(x)\\         
         = &  1\cdot(11x^3 +3x^2 +2x + 10) + 1\cdot (3x^3 +10x^2+ 3x + 10) + 0\cdot (2x^3 + 3x^2 +11x +10)\\
         = & x^3 + 5x + 7\\
P_{o}(x) = & \sum_{i=1}^{n} s_o^{(i)}L_i(x)\\         
         = & 1\cdot(11x^3 +3x^2 +2x + 10) + 1\cdot (3x^3 +10x^2+ 3x + 10) + 0\cdot (2x^3 + 3x^2 +11x +10) \\ 
         = & x^3 +5x +7 
\end{array}
$$
By design, these polynomials evaluate to the $j$-th selector constants if evaluated on the $j$-th power of $\omega$. For example $P_o(12)=1$ because $s_o^{(2)}=1$, or $P_o(8)=0$ because $s_o^{(3)}=0$ and so on. 

Using our sage definition XXX of the Lagrange base $L_H$ for the three-fuc propblem, we can double check the previous computationas follows:
\begin{sagecommandline}
sage: def linear_combination(coef, base):
....:     result = F13t(0)
....:     for i in range(len(coef)):
....:         result += coef[i] * base[i] 
....:     return result
sage: sl = [0,0,1,0] # left input selector
sage: sr = [0,0,0,0] # right input selector
sage: sm = [1,1,0,0] # multiplication selector
sage: so = [1,1,0,0] # output selector
sage: Pl = linear_combination(sl,LH)
sage: Pr = linear_combination(sr,LH)
sage: Pm = linear_combination(sm,LH)
sage: Po = linear_combination(so,LH)
sage: Pl,Pr,Pm,Po
sage: Po(F13(12)), Po(F13(8))
\end{sagecommandline}
$$
\begin{array}{rl}
S_{\sigma_1}(x) = & \sum_{i=1}^{n} \sigma(i)L_i(x) \\       
         = & 1\cdot(11x^3 +3x^2 +2x + 10) + 9\cdot (3x^3 +10x^2+ 3x + 10) \\
           & + 10\cdot (2x^3 + 3x^2 +11x +10) + 4\cdot (10x^3 + 10x^2 +10x +10)\\
    = &  7x^3 + 7x^2 + 10x + 6 \\    
S_{\sigma_2}(x) = & \sum_{i=1}^{n} \sigma(n+i)L_i(x) \\       
         = & 5\cdot(11x^3 +3x^2 +2x + 10) + 6\cdot (3x^3 +10x^2+ 3x + 10) \\
           & + 7\cdot (2x^3 + 3x^2 +11x +10) + 8\cdot (10x^3 + 10x^2 +10x +10)\\
    = &  11x^3 + 7x^2 + 3x \\ 
S_{\sigma_3}(x) = & \sum_{i=1}^{n} \sigma(2n+i)L_i(x) \\       
         = & 2\cdot(11x^3 +3x^2 +2x + 10) + 3\cdot (3x^3 +10x^2+ 3x + 10) \\
           & + 11\cdot (2x^3 + 3x^2 +11x +10) + 12\cdot (10x^3 + 10x^2 +10x +10)\\
    = &  4x^3 + 7x^2 + 7x + 7           
\end{array}
$$
By design, the first permutation polynomial evaluates to $\sigma(j)$ if evaluated on the $j$-th power of $\omega$. For example $S_{\sigma_1}(5)=1$ because $\sigma(1)=1$, or $S_{\sigma_1}(12)=9$ because $\sigma(2)=9$ and so on. Similar the second permutation polynomial evaluates to $\sigma(4+j)$ if evaluated on the $j$-th power of $\omega$. For example $S_{\sigma_2}(5)=5$ because $\sigma(5)=5$, or $S_{\sigma_2}(8)=7$ because $\sigma(7)=7$ . The same pattern holds true for the third permutation polynomial.

\begin{sagecommandline}
sage: sigma1 = [1,9,10,4]
sage: sigma2 = [5,6,7,8]
sage: sigma3 = [2,3,11,12]
sage: Psigma1 = linear_combination(sigma1,LH)
sage: Psigma2 = linear_combination(sigma2,LH)
sage: Psigma3 = linear_combination(sigma3,LH)
sage: Psigma1,Psigma2,Psigma3
sage: Psigma1(F13(5)), Psigma1(F13(12))
sage: Psigma2(F13(5)), Psigma2(F13(8))
\end{sagecommandline}

Those polynomial can then be stored in a public place, like a blockchain, because they can be publicly verified against the Plonk constraint system.
\end{example}
...
\subsubsection{Round 0}: While not explicitly stated in the original paper, the importance of proper initialization of the Fiat Shamir sponge construction
...
B(x) =& (b_3x + b_4)Z_H(x) + \sum_{i=1}^n w_{n+i}\cdot L_i(x)\\
C(x) =& (b_5x + b_6)Z_H(x) + \sum_{i=1}^n w_{2n+i}\cdot L_i(x)
\end{array}
\end{equation}
...
\begin{example}[3-fac]  In order to develop a more intuitive understanding of the first round in Plonk's prover phase, let's revisit the language $L_{3.fac_zk}$ from \examplename{} \ref{ex:L-3fac-zk}. As we know from example XXX, given instance $I$ Plonk's constraint system is satisfied by the following witness:
\begin{table}[H]
\centering
\begin{tabular}{|c|ccc|ccc|cccc|c|}
\hline
$\mathbb{H}$ & $x_l$ & $x_r$ & $x_o$ & \multicolumn{3}{c|}{$\sigma(\;x_l,\;\; x_r,\;\; x_o\;)$} & $s_l$ & $s_r$ & $s_m$ & $s_o$ & $c$ \\ 
\hline
$\omega^1$ & $2$ & $3$ & $6$ & $2$ & $3$ & $6$ & $0$ & $0$ & $1$ & $1$ & $0$ \\
$\omega^2$ & $6$ & $4$ & $11$ & $6$ & $4$ & $11$ & $0$ & $0$ & $1$ & $1$ & $0$ \\
$\omega^3$ & $11$ & $\bullet$ & $\bullet$ & $11$ & $\bullet$ & $\bullet$ & $1$ & $0$ & $0$ & $0$ & $11$ \\
$\omega^4$ & $\bullet$ & $\bullet$ & $\bullet$ & $\bullet$ & $\bullet$ & $\bullet$ & $0$ & $0$ & $0$ & $0$ & $0$ \\ 
\hline
\end{tabular}
\end{table}

In order to compute the wire polynomials $A$, $B$ and $C$ for this witness, we first have to randomly sample $6$ blinding factors from the underlying field $\F_{13}$ of the problem. We 'randomly' chose $b_1=1$, $b_2=2$, $b_3=3$, $b_4=4$, $b_5=5$ and $b_6=6$. With these choices and the Lagrange base $L_H$ from XXX and the vanishing polynom $Z_H$ from XXX, we compute
$$
\begin{array}{rl}
A(x) = & (b_1x + b_2)Z_H(x) + \sum_{i=1}^n w_i\cdot L_i(x) \\       
         = & (1\cdot x + 2)(x^4+12)+ 2\cdot(11x^3 +3x^2 +2x + 10) \\
           & + 6\cdot (3x^3 +10x^2+ 3x + 10) + 11\cdot (2x^3 + 3x^2 +11x +10) + 0\cdot (10x^3 + 10x^2 +10x +10)\\
    = &  x^5 + 2x^4 + 10x^3 + 8x^2 + 12x + 6 \\   
B(x) = & (b_3x + b_4)Z_H(x) + \sum_{i=1}^n w_{n+i}\cdot L_i(x) \\       
         = & (3\cdot x + 4)(x^4+12)+ 3\cdot(11x^3 +3x^2 +2x + 10) \\
           & + 4\cdot (3x^3 +10x^2+ 3x + 10) + 0\cdot (2x^3 + 3x^2 +11x +10) + 0\cdot (10x^3 + 10x^2 +10x +10)\\
    = &  3x^5 + 4x^4 + 6x^3 + 10x^2 + 2x + 1 \\  
C(x) = & (b_1x + b_2)Z_H(x) + \sum_{i=1}^n w_{2n+i}\cdot L_i(x) \\       
         = & (5\cdot x + 6)(x^4+12)+ 6\cdot(11x^3 +3x^2 +2x + 10) \\
           & + 11\cdot (3x^3 +10x^2+ 3x + 10) + 0\cdot (2x^3 + 3x^2 +11x +10) + 0\cdot (10x^3 + 10x^2 +10x +10)\\
    = &  5x^5 + 6x^4 + 8x^3 + 11x^2 + x + 8                
\end{array}
$$
\begin{sagecommandline}
sage: Wl = [2,6,11,0]
sage: Wr = [3,4,0,0]
sage: Wo = [6,11,0,0]
sage: A = F13t(1*t+2)*ZH + linear_combination(Wl,LH)
sage: B = F13t(3*t+4)*ZH + linear_combination(Wr,LH)
sage: C = F13t(5*t+6)*ZH + linear_combination(Wo,LH)
sage: A,B,C
\end{sagecommandline}
\end{example}
