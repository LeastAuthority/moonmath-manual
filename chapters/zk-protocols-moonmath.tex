...
To be more specific, let $\Sigma$ be an alphabet and $L$ a formal language defined over $\Sigma$. Then a \term{proof system} for language $L$ is a pair of probabilistic interactive algorithms $(P,V)$, where $P$ is called the \term{prover} and $V$ is called the \term{verifier}. 
...
If no $W$ exists, $I$ is not part of any word in $L_{C(\F)}$ and there is no way for $P$ to generate a valid assignment. If follows that the verifier will not accept any claimed proof send by $P$, which implies that the system has \term{soundness}.
...
To be more precise let $R$ be a rank-1 constraints system defined over some finite field $\F_r$. Then \term{Groth\_16 parameters} for $R$ are given by the set
\begin{equation}
...
Given some Groth\_16 parameters a \term{Groth\_16 protocol} is then a quadruple of probabilistic polynomial algorithms $(\textsc{Setup},\textsc{Prove},\textsc{Vfy},\textsc{Sim})$ such that
\begin{itemize}
...
\begin{example}[The 3-Factorization Problem] Consider the $3$-factorization problem from XXX\sme{add reference} and its associated algebraic circuit and rank-1 constraints system from  XXX\sme{add reference}. In this example, we want to agree on a parameter set $(R,r, \G_1, \G_2, e(\cdot,\cdot), g_1,g_2)$ in order to use the Groth\_16 protocol for our $3$-factorization problem. 

To find proper parameters, first observe that the circuit XXX\sme{add reference} as well as its associated R1CS $R_{3.fac\_zk}$ XXX\sme{add reference} and the derived QAP XXX\sme{add reference} are defined over the field $\F_{13}$. We therefore have $r=13$ and need pairing groups $\G_1$ and $\G_2$ of order $13$. 

From XXX\sme{add reference} we know, that the moon-math curve \texttt{BLS6\_6} has two subgroups $\G_1[13]$ and $\G_2[13]$, that are both of order $13$. The associated Weil pairing $b$ XXX\sme{add reference} is a proper bilinear map. We therefore choose those groups and the Weil pairing together with the generators $g_1 = (13,15) $ and $g_2=(7v^2,16v^3)$ of $\G_1[13]$ and $\G_2[13]$, as parameter
$$
...
The setup phase then samples $5$ random, \smelong{inverible}\sme{"invariable"?} elements $\alpha$, $\beta$,$\gamma$, $\delta$ and $s$ from the scalar field $\F_r$ of the protocol and outputs the \term{simulation trapdoor}
\begin{equation}
...
In real world applications, the simulation trapdoor is often called \term{toxic waste} of the setup-phase, while a common reference string is also called a pair of \term{proofer and verifier key}. 
...
\begin{example}[The $3$-factorization Problem] To see how the setup phase of a Groth\_16 zk-SNARK can be computed, consider the $3$-factorization problem from XXX\sme{add reference} and the parameters from XXX\sme{add reference}. As we have seen in XXX\sme{add reference} an associated quadratic arithmetic program is given by
\begin{multline*}
...
\tau = (6,5,4,3,2)
$$
and we keep this secret in order to simulate proofs later on. We are careful though to hide $\tau$ from anyone who hasn't read this book. From those values we then instantiate the common reference string XXX\sme{add reference}. Since our groups are subgroups of the \texttt{BLS6\_6} elliptic curve, we use scalar product notation instead of exponentiation. 

To compute the $\G_1$ part of the common reference string we use the logarithmic order of the group $\G_1$ XXX\sme{add reference} and the generator $g_1=(13,15)$ as well as the values from the simulation backdoor. Since $deg(T)=2$, we get:
\begin{align*}
...
\end{array}\right\}
$$
To compute the $\G_2$ part of the common reference string we use the logarithmic order of the group $\G_2$ XXX\sme{add reference} and the generator $g_2=(7v^2,16v^3)$ as well as the values from the simulation backdoor. Since $deg(T)=2$, we get:
\begin{align*}
...
$$
Given the simlutation trapdoor $\tau$ and the quadratic arithmetic program XXX\sme{add reference}, the associated common reference string of the $3$-factorization problem is given by
\begin{align*}
CRS_{\mathbb{G}_{1}} &=\left\{ \begin{array}{c}
(27,34),(26,34),(38,15),\Big((13,15),(33,34)\Big),
\Big(\mathcal{O}, (33,9)\Big)\\
...
To see that consider for example the polynomials $A_2(x)= 6x +10$ and $A_5(x)=7x+4$ from the QAP of this problem. To evaluate these polynomials in the exponent of $g_1$ and $g_2$ at the secrete point $s$, without knowing the value of $s$ (which is $2$), we can use the common reference string and equation XXX\sme{add reference}. Using the scalar product notation, instead of exponentiation, we get
\begin{align*}
...
In order to generate a zk-SNARK for this instance, the proofer first computes a valid constructive proof as explained in XXX\sme{add reference}, that is the proofer generates a proper witness $W= (W_1,\ldots,W_m)$ such that,  $(I_1,\ldots, I_n; W_1,\ldots,W_m)$ is a solution to the rank-1 constraints system $R$. 

The proofer then uses the quadratic arithmetic program and computes the polynomial $P_{(I;W)}$ as explained in XXX\sme{add reference}. They then divide $P_{(I;W)}$ by the target polynomial $T$ of the quadratic arithmetic. Since $P_{(I;W)}$ is constructed from a valid solution to the R1CS we know from XXX\sme{add reference} that it is divisible by $T$. This implies that polynomial division of $P$ by $T$ generates another polynomial $H:= P / T$, with $deg(H)<deg(T)$. 

The proofer then evaluates the polynomial $(H\cdot T)\delta^{-1}$ in the exponent of the generator $g_1$ at the secret point $s$ as explained in XXX\sme{add reference}. To see how this can be achieved, let
\begin{equation}
...
a zk-SNARK, consider the $3$-factorization problem from XXX\sme{add reference}, our protocol parameters from XXX\sme{add reference} as well as the common reference string from XXX\sme{add reference}.

Our task is to compute a zk-SNARK for the instance $I_1=11$ and its constructive proof $(W_1,W_2,W_3,W_4)=(2,3,4,6)$ as computed in XXX\sme{add reference}. As we know from XXX\sme{add reference} the associated polynomial $P_{(I;W)}$ of the quadratic arithmetic program from XXX\sme{add reference} is given by
$$
P_{(I;W)} = x^2 + x + 9
$$ 
and since in this example $P_{(I;W)}$ is identical to the target polynomial $T(x)=x^2+x + 9$, we know from XXX\sme{add reference}, that the quotient polynomial $H=P/T$ is the constant degree $0$ polynomial
$$
H(x)= H_0 \cdot x^0 = 1 \cdot x^0
$$
We therefore use $[\frac{s^0\cdot T(s)}{\delta}]g_1=(26,34)$ from our common reference string XXX\sme{add reference} of the $3$-factorization problem and compute 
\begin{align*}
...
\end{align*}
To compute this point, we have to rememer that a proofer should not be in possession of the simulation trapdoor and hence does not know what $\alpha$, $\beta$, $\delta$ and $s$ are. In order to compute this group element, the proofer therefore need the common reference string. Using the logarithmic order from XXX\sme{add reference} and the witness we get
\begin{align*}
...
\end{align*}
Since we don't know what $\alpha$, $\delta$ and $s$ are we look up $[\alpha]g_1$ and $[\delta]g_1$ from the common reference string and recall from XXX\sme{add reference} that we can evaluate $[A_j(s)]g_1$ without knowledge of the secret evaluation point $s$. According to XXX\sme{add reference} we have $[A_2(s)]g_1=(35,15)$, $[A_5(s)]g_1=(26,34)$ and $[A_j(s)]g_1=\mathcal{O}$ for all other indizes $0\leq j\leq 5$. Since $\mathcal{O}$ is the neutral element on $\G_1$, we get  
\begin{align*}
...
\end{align*}
Since we don't know what $\beta$, $\delta$ and $s$ are we look up the associated group elements from the common reference string and recall from XXX\sme{add reference} that we can evaluate $[B_j(s)]g_1$ without knowledge of the secret evaluation point $s$. Since $B_3=A_2$ as well as $B_4=A_5$, we have $[B_3(s)]g_1=(35,15)$, $[B_4(s)]g_1=(26,34)$ according to XXX\sme{add reference} and $[B_j(s)]g_1=\mathcal{O}$ for all other indices $0\leq j\leq 5$. Since $\mathcal{O}$ is the neutral element on $\G_1$, we get
\begin{align*}
...
a zk-SNARK for some given instance $I$, consider the $3$-factorization problem from XXX\sme{add reference}, our protocol parameters from XXX\sme{add reference}, the common reference string from XXX\sme{add reference} as well as the zk-SNARK $\pi=((35,15),(27,9),(7v^2,27v^3))$, which claims to be an argument of knowledge for a witness for the instance $I_1=11$.
...
\end{align*}
To compute this point, we have to remember that a verifier should not be in possession of the simulation trapdoor and hence does not know what $\alpha$, $\beta$, $\gamma$ and $s$ are. In order to compute this group element, the verifier therefore need the common reference string. Using the logarithmic order from XXX\sme{add reference} and instance $I_1$ we get
\begin{align*}
[I]g_1 & = [\frac{\beta\cdot A_{0}(s)+\alpha\cdot B_{0}(s)+C_{0}(s)}{\gamma}]g_1 \oplus [I_1][\frac{\beta\cdot A_{1}(s)+\alpha\cdot B_{1}(s)+C_{1}(s)}{\gamma}]g_1\\
...
\end{align*}  
In order to check equation XXX\sme{add reference}, observe that the target group $\G_T$ of the Weil pairing is a finite cyclic group of order $13$. Exponentiation is therefore done in modular $13$ arithmetics. Using this we evaluate the left side of equation XXX\sme{add reference} as  
$$
e([A]g_1,[B]g_2) = e((13,15),(7v^2,16v^3))^{108} =e((13,15),(7v^2,16v^3))^{4}
$$
since $\Zmod{108}{13}=4$. Similarly, we evaluate the right side of equation XXX\sme{add reference} using modular $13$ arithmetics and the exponential law $a^x\cdot a^y = a^{x+y}$. We get
\begin{align*}
...
To be more precise, let $I$ be an instance for some R1CS language $L_R$. We call a zk-SNARK for $L_R$ \term{forged} or \term{simulated}, if it passes any verification but its generation does not require the existence of a witness $W$ such that,  $(I;W)$ is a word in $L_R$. 
...
zk-SNARK for some given instance $I$, consider the $3$-factorization problem from XXX\sme{add reference}, our protocol parameters from XXX\sme{add reference}, the common reference string from XXX\sme{add reference} and the simulation trapdoor $\tau = (6,5,4,3,2)$ of that CRS.

In order to forge a zk-SNARK for instance $I_1=11$ we don't need a constructive proof for the associated rank-1 constraints system, which implies that we don't have to execute the circuit $C_{3.fac}(\F_{13})$. Instead we have to choose $2$ arbitrary elements $A$ and $B$ from $\F_{13}$ and compute $g_1^A$, $g_2^B$ and $g_1^C$ as defined in XXX\sme{add reference}. We choose $A=9$ and $B=3$ and since $\delta^{-1}=3$, we compute
\begin{align*}
[A]g_1 =& [9](13,15) = (35,15)\\
...
To see that we show that our forged SNARK passes the verification process. In order to verify $\pi_{fake}$ we proceed as in XXX\sme{add reference} and compute the curve point $g_1^I$ for the instance $I_1=11$. Since the instance is the same as in example XXX\sme{add reference}, we can parallel the computation from XXX\sme{add reference} and get
\begin{align*}
[I]g_1 & = [\frac{\beta\cdot A_{0}(s)+\alpha\cdot B_{0}(s)+C_{0}(s)}{\gamma}]g_1 \oplus [I_1][\frac{\beta\cdot A_{1}(s)+\alpha\cdot B_{1}(s)+C_{1}(s)}{\gamma}]g_1\\
       & = (35,28)
\end{align*} 
