...
\label{ex:3-fac-groth-16-params-circom} Snark.js is a JavaScript library that provides support for developing systems utilizing zero-knowledge proofs (ZKPs), including the Groth-16 protocol. In order to provide a real world example for the 3-factorization problem, we use it's Circom implementation \ref{ex:3-fac-circom}, which compiles into a representation that snark.js understands.
...
$$
Given the simulation trapdoor $\tau$ and the Quadratic Arithmetic Program \ref{ex:3-fac-QAP}, the associated \concept{Common Reference String} of the $3$-factorization problem is as follows:
\begin{align*}
CRS_{\mathbb{G}_{1}} &=\left\{ \begin{array}{c}
(27,34),(26,34),(38,15),\Big((13,15),(33,34)\Big),
\Big(\mathcal{O}, (33,9)\Big)\\
...
\label{ex:3-fac-groth-16-setup-circom} To see how real world applications handle the setup phase of a Groth\_16 zk-SNARK, consider the Circom implementation of the $3$-factorization problem \ref{ex:3-fac-circom} and Snark.js's associated parameter set from example \ref{ex:3-fac-groth-16-params-circom}. 

In Snark.js the generation of the common reference string consists of two parts, which increases flexibility in its support for other protocols. The first part depends on an upper bound of the number of constraints in the circuit, and the second part depends on the circuit itself. 

The first part is usually called \textit{powers of tau}, because it computes consecutive powers $g^\tau$, $g^{\tau^2}$, $g^{\tau^3}$, $\ldots$ of a random field element $\tau$ in the exponent of the agreed on generators of $\G_1$ and $\G_2$, where the randomness is either provided by a trusted third party, or generated in a multi party computation. 
...
After the initialization, it is possible for different parties to contribute randomness to the common reference string as follows
\begin{lstlisting}
snarkjs powersoftau contribute pot4_0000.ptau pot4_0001.ptau --name="1st_cont" -v
\end{lstlisting}
This creates a file with a new contribution and the user is prompted to enter some random text to provide an extra source of entropy. This step can be repeated for multiple users and it can be shown that it is not possible to recover the simulation trapdoor \ref{def:groth16-trapdoor}, as long as at least one contributor does not store their randomness. The correctness of any contribution file can be verified as follows:
\begin{lstlisting}
snarkjs powersoftau verify pot4_0001.ptau
\end{lstlisting}
In order to finalize the first phase some unpredictable, public randomness must be included into the setup, which was not known before the last contribution of any party. This is usually done by using the hash of the latest block in a public blockchain, or something similar:
\begin{lstlisting}
snarkjs powersoftau beacon pot4_0001.ptau pot4_beacon.ptau 0102030405060708090a0b0c00 10 -n="Final Beacon"
\end{lstlisting}
The following two commands initialize the second phase and verify it: 
\begin{lstlisting}
snarkjs powersoftau prepare phase2 pot4_beacon.ptau pot4_final.ptau -v
snarkjs powersoftau verify pot4_final.ptau
\end{lstlisting}
In our case, the randomness encoded in the power of tau ceremony can then be used for the generation of a common reference string for any circuit, which has less then $2^4$ constraints.  

Since we know from \ref{ex:3-fac-circom}, that Circom's three\_fac circuit has two constraints, we can use the randomness from this power of tau ceremony to generate a common reference string as follows:
\begin{lstlisting}
snarkjs groth16 setup three_fac.r1cs pot4_final.ptau three_fac.zkey
\end{lstlisting}
This computes a Groth-16 common reference string as defined in \ref{def:groth16-crs} and saves it to \fname{three\_fac.zkey}.
