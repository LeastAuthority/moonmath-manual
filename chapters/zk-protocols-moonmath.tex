...
According to \cite{bernhard-16}, it is not always necessary to include the initial transcript $\mathtt{T}_0$ in a Fiat-Shamir transformation. Bernhard et al. referred to systems that include $\mathtt{T}_0$ as a \term{strong Fiat-Shamir transformation}, while those that do not include it are called a \term{weak Fiat-Shamir transformation}. However, determining which case applies to a particular problem is currently not an easy task. Therefore, we have chosen to always include $\mathtt{T}_0$.
