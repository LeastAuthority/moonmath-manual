...
Projective planes are a certain type of geometry defined over some given field, that in a sense extend the concept of the ordinary Euklidean plane by including "points at infinity". 

Such an inclusion of infinity points makes them particulary useful in the description of elliptic curves, as the description of such a curve in an ordinary plane needs an additional symbol "the point at infinity" to give the set of points on the curve the structute of a group. Translating the curve into projective geometry, then includes this "point at infinity" more naturally into the set of all points on a projective plane.
...
To be more precise, let $\F$ be a field, $\F^3:=\F\times \F\times F$ the set of all three tupels over $\F$ and $x\in \F^3$ with $x=(x_1,x_2,x_3)$. Then there is exactly one \textit{line} in $\F^3$ that intersects both $(0,0,0)$ and $x$. This line is given by
\begin{equation}
...
\end{equation}
It can be shown that a projective plane over a finite field $\F_{p^m}$ contains $p^{2m}+p^m+1$ many ellements. 
...
To find $\F_3\mathbb{P}^2$, we have to compute the set of all lines in $\F_3\times \F_3\times \F_3$ that intersect $(0,0,0)$. Since those lines are parameterized by tupels $(x_1,x_2,x_3)$. We compute:
\begin{align*}
...
\end{multline*}
To understand the abiguity in projective coordinates a bit better, lets consider the point $L_{(1,2,2)}$. As this point in the projective plane is a line in $\F_3^3$, it has the projective coordinates $(1,2,2)$ as well as $(2,1,1)$, since the former coordinate give the latter, when multiplied in $\F_3$ by the factor $2$. In addition note, that for the same reasons the points $L_{(1,2,2)}$ and $L_{(2,1,1)}$ are the same, since their underlying sets are equal.
\end{example}
\begin{exercise}
