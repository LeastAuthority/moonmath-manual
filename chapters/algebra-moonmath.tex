...
\begin{example}Maybe the most basic example of a non-degenerate pairing is obtained if we take $\G_1$, $\G_2$ and $\G_3$ all to be the groups of integers with addition $(\Z,+)$. Then the following map defines a non-degenerate pairing:
$$
...
H : \{0,1\}^* \to \G
\end{equation}
Common properties of hash functions, like uniformity are desireable but not always realized in actual real Common properties of hash functions, like uniformity, are desirable but not always realized in actual real world instantiations of hash-to-group functions, so we skip those requirements for now and keep the definition very general.
