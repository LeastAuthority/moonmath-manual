...
\end{exercise}
\begin{exercise}\label{ex:modulus-prime-group}(Hard) Let $p\in\mathbb{P}$ be prime number and $(\Z_p^*,\cdot)$ the finite group from exercise \ref{ex:Zn*}. Show that $(\Z_p^*,\cdot)$ is cyclic.
\end{exercise}
...
\begin{equation}
\cdot : \; \F_{p^m}\times \F_{p^m} \to \F_{p^m}\; , \textstyle\; (\sum_{j=0}^{m-1} a_j x^j,\sum_{j=0}^{m-1} b_j x^j)\mapsto \Zmod{\left(\sum _{n = 0} ^{2(m-1)} \sum _{i = 0} ^{n}{a} _{i }{{b} _{n-i}}{x} ^{n}\right)}{P}
\end{equation}
