...
Distilling these rules to the smallest independent list of properties and making them abstract, we arrive at the definition of a commutative group:

\begin{definition}
A \term{commutative group} $(\G,\cdot) $ is a set $ \G$, together with a \term{map} $ \cdot$. The map, also denoted as $\cdot:\G \times \G \to \G $ and called the \term{group law}, combines two elements of the set $ \G$ into a third one such that the following properties hold:
\begin{itemize}
...
\end{itemize}
If $(\G,\cdot)$ is a group and $\G'$ is a subset of $\G$, such that the restriction $\cdot: \G'\times \G' \to \G'$ is a group law on $\G'$, then $(\G',\cdot)$ is called a \term{subgroup} of $(\G,\cdot)$.
\end{definition}
