...
\begin{center}
  ...
  \end{tabular}
\end{center}
So the multiplicative inverse of $3$ in $\Z_5$ is $2$, and, indeed if we compute $3\cdot 2$, we get $1$ in $\F_5$.
\end{example}
...
\end{align*}
\end{example}
\begin{exercise} Consider the prime field $\F_{13}$. Find the set of all pairs $(x,y)\in \F_{13}\times \F_{13}$ that satisfy the following equation:
$$
...
To be more precise, let $P\in F_p[x]$ be an irreducible polynomial of degree $m$ with coefficients from the given prime field $\F_p$. Then the underlying set $\F_{p^m}$ of the extension field is given by the set of all polynomials with a degree less than $m$ as follows:
\begin{equation}
