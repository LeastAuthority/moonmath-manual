...
\begin{example}[Naive cyclic group hash] Let $\G$ be a finite cyclic group. If the task is to implement a hash-to-$\G$ function, one immediate approach might be the following:

Choose an ordinary hash function $H:\{0,1\}^*\to \{0,1\}^k$ for some parameter $k$
and interpret the image $H(s)$ as the binary representation of an integer, that is consider $H(s)\in \Z$. Then choose a generator $g$ of $\G$ and define the new hash-to-$\G$ function as a concatenation of the exponential map $g^{(\cdot)}$ of $g$ with the hash function $H$, that is define the new hash-to-$\G$ function as
$$
H_{g} : \{0,1\}^* \to \G:\; s \mapsto g^{H(s)} 
$$
...
To see that, assume that $\G$ is of order $r$ and that $H(s)\in\Z_r$ has a multiplicative inverse in modular $r$ arithmetics. In that case we can compute $x=H(t)\cdot H(s)^{-1}$ and have found a discrete log relation between the group hash value, that is some $x$ with $H_g(t) = (H_g(s))^x$ since
\begin{align*}
H_g(t) & = (H_g(s))^x & \Leftrightarrow \\
g^{H(t)} & = g^{H(s)\cdot x} & \Leftrightarrow \\
g^{H(t)} & = g^{H(t)}
\end{align*}
\end{example}
Applications where discrete log relations between hash values are undesireable therefore need different approaches and many of those approaches start with a way to hash into the sets $\Z_n$ of modular $n$ arithmetics. 
\paragraph{Hashing into modular arithmetics} One of the most widely used applications of hash-into-group functions are hash functions that map into the set $\Z_n$ of modular $n$ arithmetics for some modulus $n$. Different approaches to construct such a function are known, but but probably the two most used are based on the insight that the images of arbitrary hash functions can be interpreted as binary representations of integers.

The first method of hashing into $\Z_n$ is constructed by observing, that if $n$ is a modulus, with a bit-length of $k$, then every binary string $(b_0,b_1,\ldots,b_{k-1})$ of length $k-1$ can be interpreted as an element $x\in\Z_n$, by defining
\begin{equation}
x = b_0\cdot 2^0 + b_1\cdot 2^1 + \ldots + b_{k-1}\cdot 2^{k-1}
\end{equation} 
since by construction $x$ is always smaller then $n$ and hence $x\in\{0,1,\ldots,n-1\}$. From this follows that if $H:\{0,1\}^*\to\{0,1\}^{k-1}$ is a hash function, then a hash-to-group function can be constructed by
\begin{equation}
H_{Z_n}: \{0,1\}^* \to \Z_n: \; s \mapsto 
H(s)_0\cdot 2^0 + H(s)_1\cdot 2^1 + \ldots + H(s)_{k-1}\cdot 2^{k-1}
\end{equation}
where $H(s)_j$ means the $j$'s bit of the image binary string $H(s)$ of the original binary hash function. 

A drawback of this hash function is that the distribution of the hash values in $\Z_n$ is not even. In fact if $n-2^{k-1}\neq 0$, then by design the function will never hash onto values larger then $2^{k-1}$. An advantage is that potential properties like preimage or collision resistence of the original hash function $H(\cdot)$ are preserved. 

Despite the fact that distribution is not even, it is sometimes used in applications. The reasoning is, that if $k$ is a cryptographically reasonable number like $128$ or larger, the number of elements $n-2^{k-1}$ that the hash function misses, is tiny compared to the numbers that it hashes to and the loss in security is only a single bit.

The second widely used method of hashing into $\Z_n$ is constructed by observing, that if $n$ is a modulus, with a bit-length of $k_1$ and $H:\{0,1\}^*\to {0,1}^{k_2}$ is a hash function that produces digests of size $k_2$, with $k_2>k_1$, then a hash-to-group function can be constructed by interpreting the image of $H$ as binary representation of a integer and then take the modulus by $n$ to map into $\Z_n$. To be more precise 
\begin{equation}
H'_{Z_n}: \{0,1\}^* \to \Z_n: \; s \mapsto 
\Zmod{\left(H(s)_0\cdot 2^0 + H(s)_1\cdot 2^1 + \ldots + H(s)_{k_2}\cdot 2^{k_2}\right)}{n}
\end{equation}
where $H(s)_j$ means the $j$'s bit of the image binary string $H(s)$ of the original binary hash function. 

A drawback of this hash function is that computing the modulus requires some computational effort. So $H'_{Z_n}$ is more expensive then $H_{\Z_n}$.

Also something to look out for, is the fact that the distribution of the hash values in $\Z_n$ might not even, depending on the size of the difference $k_2-k_1$.

An advantage is that potential properties like preimage or collision resistence of the original hash function $H(\cdot)$ are preserved and distributen is even provided that $k_1$ and $k_2$ are choosen properly. 

A thid method that can sometimes be found in implementations is the so called \textit{try and increment method}. To underatand this method, observe that if $n$ is a modulus, with an bit-length of $k$ and $H:\{0,1\}^*\to {0,1}^{k}$ is a hash function, then the integer
\begin{equation}
x = H(s)_0\cdot 2^0 + H(s)_1\cdot 2^1 + \ldots + H(s)_{k-1}\cdot 2^{k}
\end{equation}
is an element of $\Z_n$, whenever $x<n$. So hashing to $\Z_n$ can be achieved by computing $x$ and then try to see if $x\in\Z_n$. If this is the case than the hash is done and if not the string $s$ is modified and the process is repeated. For example a suteable, deterministic modificatio could be to extend the string by some $k_2$-bit counter. A try and increment algorithm would then work like this:
\begin{algorithm}\caption{Hash-to-$\Z_n$}
\label{alg_projective_group_law}
\begin{algorithmic}[0]
\Require $n \in \Z$ with $bit\_length(n)=k$ and $s\in\{0,1\}^*$
...
If $k$ is suficiently large, the probability for $x<n$ is very high and the repeat loop will almost always be executed a single time only. A drawback is however that the probability to execute the loop multiple times is not zero. 
...
To be more precise, let $\F$ be a field, $\F^3:=\F\times \F\times F$ the set of all three tuples over $\F$ and $x\in \F^3$ with $x=(x_1,x_2,x_3)$. Then there is exactly one \textit{line} in $\F^3$ that intersects both $(0,0,0)$ and $x$. This line is given by
\begin{equation}
L_x := \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F\}
\end{equation}
...
\begin{equation}
\F\mathbb{P}^2:=\{L_x\;|\; x\in \F^3\text{ with } x\neq (0,0,0)\}
\end{equation}
It can be shown that a projective plane over a finite field $\F_{p^m}$ contains $p^{2m}+p^m+1$ many elements.

To understand why $L_x$ is called a line, consider the situation, where the underlying field $\F$ are the real numbers $\mathbb{R}$. Then $\mathbb{R}^3$ can be seen as the three dimensional space and $L_{(x,y,z)}$ is then an ordinary line in this 3-dimensional space that intersects zero and the point with coordinates $x$, $y$ and $z$.
...
It follows from this that points $L_x\in \F\mathbb{P}^2$ are not simply described by fixed coordinates $(x_1,x_2,x_3)$, but by \textit{sets of coordinates} rather, where two different coordinates $(x_1,x_2,x_3)$ and $(x'_1,x'_2,x'_3)$, with $x_j\neq 0$ and $x_j'\neq 0$ describe the same point, if and only if there is some field element $k$, such that $(x_1,x_2,x_3) = (k\cdot x'_1,k\cdot x'_2,k\cdot x'_3)$. Descriptions like that are called \textbf{projective coordinates}.

\begin{notation}[Projective coordinates]
%https://math.mit.edu/classes/18.783/2017/Lecture1.pdf
To distingush the (equivalence class) of projective coordinates of a line $L_x$ from the coordinates of $x$, we write $[x_1:x_2:x_3]$ for the set of all projective coordinates of $L_x$. Coordinates of the form $[x_1:x_2:1]$ are descriptions of so called \textbf{affine points} and coordinates of the form $[x_1:x_2:0]$ are descriptions of so called \textbf{points at infinity}. In particular the projective coordinate $[1:0:0]$ describes the so called \textbf{line at infinity}.
\end{notation}
...
\begin{align*}
L_{(0,0,1)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (0,0,1), (0,0,2)\}\\
L_{(0,0,2)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (0,0,2), (0,0,1)\}
          = L_{(0,0,1)}\\
L_{(0,1,0)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (0,1,0), (0,2,0)\}\\
L_{(0,1,1)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (0,1,1), (0,2,2)\}\\
L_{(0,1,2)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (0,1,2), (0,2,1)\}\\
L_{(0,2,0)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (0,2,0), (0,1,0)\}
          = L_{(0,1,0)}\\
L_{(0,2,1)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (0,2,1), (0,1,2)\}
          = L_{(0,1,2)}\\
L_{(0,2,2)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (0,2,2), (0,1,1)\}
          = L_{(0,1,1)}\\
L_{(1,0,0)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (1,0,0), (2,0,0)\}\\
L_{(1,0,1)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (1,0,1), (2,0,2)\}\\
L_{(1,0,2)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (1,0,2), (2,0,1)\}\\
L_{(1,1,0)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (1,1,0), (2,2,0)\}\\
L_{(1,1,1)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (1,1,1), (2,2,2)\}\\
L_{(1,1,2)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (1,1,2), (2,2,1)\}\\
L_{(1,2,0)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (1,2,0), (2,1,0)\}\\
L_{(1,2,1)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (1,2,1), (2,1,2)\}\\
L_{(1,2,2)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (1,2,2), (2,1,1)\}\\
L_{(2,0,0)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (2,0,0), (1,0,0)\}
          = L_{(1,0,0)}\\
L_{(2,0,1)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (2,0,1), (1,0,2)\}
          = L_{(1,0,2)}\\
L_{(2,0,2)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (2,0,2), (1,0,1)\}
          = L_{(1,0,1)}\\
L_{(2,1,0)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (2,1,0), (1,2,0)\}
          = L_{(1,2,0)}\\
L_{(2,1,1)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (2,1,1), (1,2,2)\}
          = L_{(1,2,2)}\\
L_{(2,1,2)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (2,1,2), (1,2,1)\}
          = L_{(1,2,1)}\\
L_{(2,2,0)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (2,2,0), (1,1,0)\}
          = L_{(1,1,0)}\\
L_{(2,2,1)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (2,2,1), (1,1,2)\}
          = L_{(1,1,2)}\\
L_{(2,2,2)} &= \{(k\cdot x_1,k\cdot x_2, k\cdot x_3)\;|\; k\in\F_3\}
          = \{(0,0,0), (2,2,2), (1,1,1)\}
          = L_{(1,1,1)}
\end{align*}
Those lines define the $13$ points in the projective plane $\F_3\mathbb{P}$ as follows
\begin{multline*}
\F_3\mathbb{P} = \{ L_{(0,0,1)}, L_{(0,1,0)}, L_{(0,1,1)}, L_{(1,0,0)}, L_{(1,0,1)}, \\ L_{(1,0,2)}, L_{(1,1,0)}, L_{(1,1,1)}, L_{(1,1,2)}, L_{(1,2,0)}, L_{(1,2,1)}, L_{(1,2,2)}\}
\end{multline*}
To understand the ambiguity in projective coordinates a bit better, lets consider the point $L_{(1,2,2)}$. As this point in the projective plane is a line in $\F_3^3$, it has the projective coordinates $(1,2,2)$ as well as $(2,1,1)$, since the former coordinate give the latter, when multiplied in $\F_3$ by the factor $2$. In addition note, that for the same reasons the points $L_{(1,2,2)}$ and $L_{(2,1,1)}$ are the same, since their underlying sets are equal.
\end{example}
\begin{exercise}
