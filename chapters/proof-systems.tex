...
In [XXX] Jens Groth provids a method that can transform those proofs into zero-knowledge succinct non interactive arguments of knowledge, assuming that cryptographically secure type III pairung groups $(\G_1,\G_2,\G_T,b)$ exists. 

The arguments that this method computes are of constant size and consist of $2$ elements from $G_1$ and a single element from $\G_2$, regardless of the size of the witness. They are zero-knowledge in the sense, that the verifier learns nothing about the witness, besides the fact that the instnce, witness pair is a proper word in the language of the problem.
...
To be more precise let $L$ be a language and $R$ a rank-1 constraints system defined over some field $F_r$. Then every implementation of the protocol, assumes the existence of two finite cyclic groups $\G_1$ and $\G_2$ of order $r$ and a bilinear pairing $b: \G_1 \times \G_2 \to \G_T$ for some target group $\G_T$ as well as a generator $g_1$ of $\G_1$ and a generator $g_2$ of $\G_2$. 
...
\begin{example}[The 3-Factorization Problem] Consider the $3$-factorization problem from XXX and its associated algebraic circuit and rank-1 constraints system from  XXX. In this example, we want to agree on biliniear pairing groups, such that we can use the Groth16 protocol to convert constructive proofs for this problem as described in XX into zk-SNARKs. 

To define a proper environment, first observe that the circuit as well as the associated R1CS and its QAP are defined over the field $\F_{13}$. We therefore need pairing groups $\G_1$ and $\G_2$, such that both groups are of order $13$. 

From XXX we know, that the moon-math curve \texttt{BLS6\_6} has two $13$-torsion subgroups $\G_1$ and $\G_2$, that are both of order $13$ and that the Weil pairing $b$ on those groups is a proper bilinear map. We therefore choose those groups and their WEil pairing together with the generators $g_1 = (13,15) $ and $g_2=(7v^2,16v^3)$ of $\G_1$ and $\G_2$, respectively. The target group is then given by $\mathbb{G}_T = \F_{43^6}^*$.
...
To be more precise, let $L$ be a language defined by some rank-1 constraints system $R$, such that a constructive proof of knowledge for an instance $(I_1,\ldots,I_n)$ in $L$ consists of a witness $(W_1,\ldots,W_m)$. Let $QAP(R) = \left\{T\in \F[x],\left\{A_j,B_j,C_j\in \F[x]\right\}_{h=0}^{n+m}\right\}$ be a quadratic arithmetic program associated to $R$ and $\{\G_1, G_2, g_1, g_2, \F_r\}$ be the set of parameters of the protocol. 

The setup phase then samples $5$ random elements $\alpha$, $\beta$,$\gamma$, $\delta$ and $s$ from the scalar field $\F_r$ of the protocol and outputs the \textbf{simulation trapdoor}
\begin{equation}
...
\end{align*}
Common reference strings depend on random elements and are therefor not unique to the proplem. Any language can have more then one common reference string. The  size of a common reference string is linear in the size of the instance and the size witness.
...
In real world applications, a simulation trapdoor is often called the \textit{toxic waste} of the protocol, while a common reference string is called a pair of \textit{proofer and verifier key}. 

In order to make the protocol secure the setup needs to be executed in a way, such that it is guranteed that the simulation trapdoor is deleted. Anyone in possesion of it can simulate proofs. The most simple approach to achieve this is by a so called \textit{trusted third party}, where the trust assumes that the party generate the common reference string precisely as defined and deletes the simulation backdoor.
...
Proofer and verifier key. Public/private verification
\begin{example}[The $3$-factorization Problem] To see how the setup phase of Groth16 zk-SNARK can be computed, consider the $3$-factorization problem from XXX and our protocol parameters from XXX. As we have seen in XXX an associated quadratic arithmetic program is given by
\begin{multline*}
QAP(R_{3.fac\_zk}) =\\
...
Our simulation backdoor is therefore given by
$$
\tau = (6,5,4,3,2)
$$
...
[A_5(s)]g_2 & = [7\cdot s^1 + 4\cdot s^0] g_1 \\
     ...
     & = (16v^2,28v^3)     
\end{align*}
Except the target polynomial $T$ all polynomial of the quadratic arithmetic program can be evaluated in the exponent this way.
\end{example}
\paragraph{The Proofer Phase} We assume that any proofer has access to the rank-1 constraints system of the problem in addition with some algorithm, that tells the proofer how to compute constructive proofs for the R1CS. In addition the proofer needs accces to a  common reference string and its associated quadratic arithmetic program. 

Assuming some inatance $(I_1,\ldots, I_n)$ is given. In order to generate a zk-SNARK for this instance, the proofer first needs to compute a valid constructive proof as explained in XXX, that is the proofer needs to compute a proper witness $W$. The proofer then uses the quadratic arithmetic program and computes the polynomial $P_{(I;W)}$ and divides it by the target polynomial $T$, to get another polynomial $H:= P / T$, with $deg(H)<deg(T)$. 

Let $(W_1,\ldots,W_m)$ be the witness from the constructive proof. The proofer then samples two random field elements $r,t\in \F_r$ and generates a zk-SNARK $\pi = (g_1^A, g_1^C, g_2^B)$, where
\begin{align*}
...
\end{align*}  
As we have seen in XXX, the group elements $g_1^{A_j(s)}$, $g_2^{B_j(s)}$, can be derived from the common reference string. In fact those points only have to be computed once and can then be published and reused for multiple proof generations. All other group elements are part of the common reference string. 


Proofs are not unique for a given witness. randomness. This is important, because if one proof is knwon to proof a specific witness one can not see that other proof are for the same witness

\begin{example} T
Two proofs for Different witnesses

impossible to tell if the proofs are for the same witness of for different ones
\end{example}
...
\paragraph{Proof Matability} Anti-Groth+Maller
\begin{comment}
